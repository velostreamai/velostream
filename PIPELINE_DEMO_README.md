# FerrisStreams Complete Pipeline Demo

This demo showcases FerrisStreams' end-to-end **File → Kafka → File** processing pipeline with financial transaction data, demonstrating:
...
```bash
# Start Kafka using the demo compose file
...
# The demo will:
# 1. Generate sample financial transaction data
# 2. Read from CSV files using FileDataSource
# 3. Process with exact DECIMAL precision
# 4. Stream through Kafka with performance metrics
# 5. Write results to compressed JSON files using FileSink
```
