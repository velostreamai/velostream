...
use ferrisstreams::{ProducerBuilder, KafkaConsumer, KafkaAdminClient};
...
impl SimpleBufferPool {
    ...
    fn get_buffer(&self) -> Vec<u8> {
        self.buffers.pop().unwrap_or_else(|| vec![0u8; PAYLOAD_SIZE])
    }
            ...
            .for_each(|message_result| async {
                match message_result {
                    Ok(message) => {
                        ...
                        let _checksum = data.iter().fold(0u64, |acc, &byte| acc.wrapping_add(byte as u64));
