...
use velostream::{Headers, JsonSerializer, FastConsumer, Message, ProducerBuilder}; // Add this import for stream extension traits
...
async fn run_performance_test(
    ...
    let consumer = FastConsumer::<String, PerformanceTestMessage>::with_config(
        consumer_config,
        JsonSerializer,
        JsonSerializer,
    )?;
