...
use ferrisstreams::ferris::sql::schema::{FieldDefinition, Schema, SchemaMetadata, CompatibilityMode};
...
impl TestMessage {
    ...
    fn to_stream_record(&self) -> StreamRecord {
        ...
        fields.insert("price".to_string(), FieldValue::ScaledInteger((self.price * 10000.0) as i64, 4));
        fields.insert("volume".to_string(), FieldValue::Integer(self.volume));
        fields.insert("timestamp".to_string(), FieldValue::Integer(self.timestamp as i64));
...
async fn benchmark_kafka_abstraction() -> PerformanceStats {
    ...
    for i in 0..OPERATION_COUNT {
        ...
        let uri = format!("kafka://localhost:9092/test-topic-{}?group_id=test-group-{}", 
                         i % 100, i % 10);
...
async fn benchmark_record_transformation() -> PerformanceStats {
    ...
    let test_messages: Vec<_> = (0..BATCH_SIZE).map(|i| TestMessage::new(i as u64)).collect();
    ...
    for batch in 0..(OPERATION_COUNT / BATCH_SIZE as u64) {
        ...
        let _records: Vec<_> = test_messages
            ...
            .map(|(i, msg)| {
                ...
                record.fields.insert(
                    "batch_id".to_string(),
                    FieldValue::Integer(batch as i64)
                );
                record.fields.insert(
        ...
        for record in &sample_records {
            for (field_name, field_value) in &record.fields {
                let data_type = match field_value {
                    ...
                    FieldValue::Map(_) => DataType::Map(Box::new(DataType::String), Box::new(DataType::String)), // String-to-String map
...
fn calculate_overhead(baseline: &PerformanceStats, test: &PerformanceStats) -> f64 {
    if baseline.avg_operation_time_us > 0 {
        ((test.avg_operation_time_us as f64 - baseline.avg_operation_time_us as f64) 
         / baseline.avg_operation_time_us as f64) * 100.0
    } else {
        0.0
    }
}
...
async fn main() {
    ...
    if abstraction_overhead < 5.0 {
        ...
        println!("⚠️  WARNING: Abstraction overhead is elevated ({:.2}%)", abstraction_overhead);
    } else {
        println!("❌ FAIL: Abstraction overhead is too high ({:.2}%)", abstraction_overhead);
    }
    
    // URI parsing performance
    if uri_parsing_stats.avg_operation_time_us < 100 {
        println!("✅ PASS: URI parsing is fast ({}μs)", uri_parsing_stats.avg_operation_time_us);
    } else {
        println!("⚠️  URI parsing performance: {}μs", uri_parsing_stats.avg_operation_time_us);
    }
    
    // Record transformation performance
    let records_per_second = (BATCH_SIZE as f64) * record_transformation_stats.ops_per_second;
    println!("📊 Record transformation: {:.0} records/second", records_per_second);
    ...
    if schema_discovery_stats.avg_operation_time_us < 1000 {
        println!("✅ PASS: Schema discovery is efficient ({}μs)", schema_discovery_stats.avg_operation_time_us);
    } else {
        println!("⚠️  Schema discovery performance: {}μs", schema_discovery_stats.avg_operation_time_us);
    }
    ...
    if abstraction_overhead < 10.0 { passed_tests += 1; }
    if uri_parsing_stats.avg_operation_time_us < 100 { passed_tests += 1; }
    if records_per_second > 50_000.0 { passed_tests += 1; }
    if schema_discovery_stats.avg_operation_time_us < 1000 { passed_tests += 1; }
    if kafka_abstraction_stats.ops_per_second > 1000.0 { passed_tests += 1; }
