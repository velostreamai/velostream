    ...
    F: FnMut() -> ()
{
    ...
    println!();
    println!("  ⏱️  {}ms total, {}μs per operation", 
             duration.as_millis(), 
...
async fn main() {
    ...
    let record_transformation_time = benchmark_operation("Record Transformation", || {
        let records: Vec<_> = (0..BATCH_SIZE).map(|i| create_test_record(i as u64)).collect();
    ...
    let record_transform_ops_per_sec = (QUICK_TEST_COUNT * BATCH_SIZE as u64) as f64 / record_transformation_time.as_secs_f64();
    
    println!("URI parsing: {:.0} URIs/second", uri_parsing_ops_per_sec);
    println!("Source creation: {:.0} sources/second", source_creation_ops_per_sec);
    println!("Record transformation: {:.0} records/second", record_transform_ops_per_sec);
    ...
    if uri_parsing_ops_per_sec > 10_000.0 {
        ...
        println!("⚠️  URI parsing performance: {} ops/sec (target: >10k)", uri_parsing_ops_per_sec as u64);
    }
    ...
    if source_creation_ops_per_sec > 1_000.0 {
        ...
        println!("⚠️  Source creation performance: {} ops/sec (target: >1k)", source_creation_ops_per_sec as u64);
    }
    ...
    if record_transform_ops_per_sec > 50_000.0 {
        ...
        println!("⚠️  Record transformation performance: {} records/sec (target: >50k)", record_transform_ops_per_sec as u64);
    }
