...
use ferrisstreams::ferris::datasource::config::{FileFormat, SinkConfig, SourceConfig};
use ferrisstreams::ferris::datasource::file::{FileDataSource, FileSink, FileSinkConfig, FileSourceConfig, CompressionType};
use ferrisstreams::ferris::datasource::traits::{DataSink, DataSource, DataWriter};
use ferrisstreams::ferris::kafka::{KafkaConsumer, KafkaProducer, JsonSerializer};
...
async fn setup_demo_environment() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  ✅ Created {} with sample transaction data", DEMO_CSV_PATH);
...
fn generate_sample_transactions_csv() -> String {
    let mut csv = String::from("transaction_id,customer_id,amount,currency,timestamp,merchant_category,description\n");
    
    let customers = ["CUST001", "CUST002", "CUST003", "CUST004", "CUST005"];
    let categories = ["grocery", "gas", "restaurant", "shopping", "entertainment", "utilities"];
    let descriptions = [
        "Whole Foods Market", "Shell Gas Station", "McDonald's", "Amazon Purchase", 
        "Netflix Subscription", "PG&E Electric", "Target Store", "Starbucks Coffee"
    ];
    ...
    for i in 1..=100 {
        ...
        let timestamp = SystemTime::now()
            ...
            .unwrap()
            .as_secs() - (24 * 3600) + (i * 864) as u64; // Spread over 24 hours
...
async fn run_file_to_kafka_pipeline(metrics: Arc<PipelineMetrics>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Ok(Some(record)) = reader.read().await {
        ...
        match process_transaction_record(&record) {
            Ok(processed_record) => {
                ...
                // Send to Kafka
                let key = processed_record.fields.get("transaction_id")
                    .and_then(|v| match v {
...
async fn run_kafka_to_file_pipeline(metrics: Arc<PipelineMetrics>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        match amount_field {
            FieldValue::String(amount_str) => {
                if let Ok(amount_float) = amount_str.parse::<f64>() {
                    ...
                    processed_fields.insert("amount_cents".to_string(), 
...
async fn monitor_pipeline_performance(metrics: Arc<PipelineMetrics>) {
    ...
    loop {
        ...
        println!("   📖 Records Read:      {:6} (+{}/s)", current_read, read_rate);
        println!("   ⚙️  Records Processed:  {:6} (+{}/s)", current_processed, process_rate);
        println!("   💾 Records Written:    {:6} (+{}/s)", current_written, write_rate);
        println!("   💰 Total Amount:      ${:10.2}", (total_amount as f64) / 100.0);
...
async fn display_final_results(metrics: &PipelineMetrics, duration: Duration) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   💰 Total Amount:     ${:.2}", (total_amount as f64) / 100.0);
    println!("   ❌ Total Errors:     {}", total_errors);
    println!();
    
    if duration_secs > 0.0 {
        println!("📈 Performance Metrics:");
        println!("   📖 Read Throughput:    {:.1} records/sec", total_read as f64 / duration_secs);
        println!("   ⚙️  Process Throughput: {:.1} records/sec", total_processed as f64 / duration_secs);
        println!("   💾 Write Throughput:   {:.1} records/sec", total_written as f64 / duration_secs);
        println!("   💰 Amount Throughput:  ${:.0}/sec", (total_amount as f64) / 100.0 / duration_secs);
        
        if total_errors > 0 {
            println!("   ❌ Error Rate:         {:.2}%", (total_errors as f64 / total_read as f64) * 100.0);
        }
        println!();
    }
    ...
    if let Ok(content) = tokio::fs::read_to_string(DEMO_OUTPUT_PATH).await {
        ...
        for (i, line) in lines.iter().enumerate() {
            if let Ok(json) = serde_json::from_str::<serde_json::Value>(line) {
                println!("   {}. {}", i + 1, serde_json::to_string_pretty(&json).unwrap_or_else(|_| line.to_string()));
            }
        }
