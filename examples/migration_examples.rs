...
use crate::ferris::sql::{
    config::{StreamingConfig, CircuitBreakerConfig, ResourceLimits, WatermarkConfig, EmitMode},
    execution::{
...
pub mod financial_trading {
    ...
    impl LegacyTradingProcessor {
        ...
        pub fn process_trade(&mut self, trade: Trade) -> Result<(), String> {
            ...
            let total_cost = position.quantity as f64 * position.avg_price + 
    ...
    impl EnhancedTradingProcessor {
        ...
        pub async fn process_trade(&mut self, trade: Trade) -> Result<(), StreamingError> {
            ...
            self.resource_manager.check_allocation("total_memory", trade_size)?;
            self.resource_manager.update_resource_usage("active_trades", self.processed_count + 1)?;
            
            // Step 2: Watermark processing
            let processing_time = SystemTime::now();
            if self.config.enable_watermarks {
                let is_late = self.watermark_manager.is_late_data(&trade.timestamp, &processing_time);
                if is_late {
            ...
            let result = self.circuit_breaker.execute(|| async {
                self.process_trade_internal(trade.clone()).await
            }).await;
            ...
            match result {
                Ok(_) => {
                    ...
                    self.resource_manager.record_processing_time("trade_processing", processing_duration)?;
                    
                    // Update resource usage after successful processing
                    self.resource_manager.update_memory_usage("positions", 
        ...
        async fn process_trade_internal(&mut self, trade: Trade) -> Result<(), StreamingError> {
            ...
            let current_value = position.quantity.checked_mul(position.avg_price as i64)
            ...
            let trade_value = trade.quantity.checked_mul(trade.price as i64)
                .ok_or_else(|| StreamingError::ProcessingError {
        ...
        pub fn get_metrics(&self) -> TradingMetrics {
            ...
            TradingMetrics {
                ...
                memory_usage: resource_usage.get("total_memory").map(|m| m.current).unwrap_or(0),
...
pub mod iot_sensors {
    ...
    impl LegacySensorProcessor {
        ...
        pub fn process_reading(&mut self, reading: SensorReading) -> Result<(), String> {
            let aggregate = self.aggregates.entry(reading.sensor_id.clone()).or_insert(SensorAggregate {
    ...
    impl EnhancedSensorProcessor {
        ...
        pub async fn process_reading(&mut self, reading: SensorReading) -> Result<(), StreamingError> {
            // Resource monitoring
            let reading_size = std::mem::size_of_val(&reading) as u64;
            self.resource_manager.check_allocation("total_memory", reading_size)?;
            
            // Watermark processing
            let processing_time = SystemTime::now();
            if self.config.enable_watermarks {
                let is_late = self.watermark_manager.is_late_data(&reading.timestamp, &processing_time);
                if is_late {
                    self.late_data_count += 1;
                    log::warn!("Late sensor data from {}: {:?}", reading.sensor_id, reading.timestamp);
            ...
            let result = self.circuit_breaker.execute(|| async {
                self.process_reading_internal(reading.clone()).await
            }).await;
            ...
            match result {
                Ok(_) => {
                    ...
                    self.resource_manager.update_resource_usage("sensor_aggregates", 
                        self.aggregates.len() as u64)?;
                ...
                Err(StreamingError::CircuitBreakerOpen { .. }) => {
                    ...
                    Err(StreamingError::CircuitBreakerOpen { 
                        recovery_time: SystemTime::now() + self.config.circuit_breaker_config.recovery_timeout 
                    })
                },
                Err(StreamingError::ResourceExhausted { resource_type, .. }) => {
                    log::warn!("Resource exhausted: {} - cleaning sensor buffers", resource_type);
        ...
        async fn process_reading_internal(&mut self, reading: SensorReading) -> Result<(), StreamingError> {
        ...
        async fn process_late_reading(&mut self, reading: SensorReading) -> Result<(), StreamingError> {
        ...
        async fn cleanup_sensor_buffers(&mut self) -> Result<(), StreamingError> {
            ...
            self.aggregates.retain(|_, aggregate| aggregate.last_updated > cutoff_time);
...
pub mod simple_migration {
    ...
    impl MigrationExample {
        ...
        pub async fn process_records(&mut self, records: Vec<SimpleRecord>) -> Result<usize, StreamingError> {
            let mut processed = 0;
            
            // Create processors based on configuration
            let mut circuit_breaker = if self.config.enable_circuit_breakers {
                Some(CircuitBreaker::new(self.config.circuit_breaker_config.clone()))
            } else {
                None
            };
            ...
            for record in records {
                ...
                let process_result = if let Some(ref cb) = circuit_breaker {
                    cb.execute(|| async {
                        Self::process_single_record(record.clone()).await
                    }).await
                } else {
