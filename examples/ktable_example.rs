...
use ferrisstreams::ferris::kafka::producer_config::{ProducerConfig, AckMode};
...
async fn create_user_profile_table() -> Result<KTable<String, UserProfile, JsonSerializer, JsonSerializer>, Box<dyn std::error::Error>> {
...
async fn display_user_profiles(user_table: &KTable<String, UserProfile, JsonSerializer, JsonSerializer>) {
...
async fn process_order_stream(user_table: KTable<String, UserProfile, JsonSerializer, JsonSerializer>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let enriched_producer = KafkaProducer::with_config(
        producer_config,
        JsonSerializer,
        JsonSerializer,
    )?;
    ...
    while let Some(message_result) = stream.next().await {
        match message_result {
            Ok(message) => {
                ...
                println!("\n📦 Processing order: {} for user {}", order.order_id, order.user_id);
                ...
                match user_table.get(&order.user_id) {
                    Some(user_profile) => {
                        ...
                        let enriched_order = EnrichedOrder {
                            ...
                            discount_eligible: user_profile.subscription_tier == "premium" || user_profile.subscription_tier == "enterprise",
                        };
                        
                        println!("✨ Enriched order for {} ({}) - {} tier, discount: {}", 
                            enriched_order.user_name,
                            enriched_order.user_email,
                            enriched_order.subscription_tier,
                            if enriched_order.discount_eligible { "eligible" } else { "not eligible" }
                        );
                        
                        // Send enriched order to output topic
                        match enriched_producer.send(
                            Some(&enriched_order.order_id),
                            &enriched_order,
                            Headers::new(),
                            None
                        ).await {
                            Ok(_) => println!("✅ Enriched order sent to {}", ENRICHED_ORDERS_TOPIC),
                            Err(e) => eprintln!("❌ Failed to send enriched order: {:?}", e),
                        }
                    }
                    None => {
                        println!("⚠️  User profile not found for user: {} (order: {})", 
...
async fn populate_sample_data() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let producer = KafkaProducer::with_config(
        producer_config,
        JsonSerializer,
        JsonSerializer,
    )?;
...
async fn generate_sample_orders() -> Result<(), Box<dyn std::error::Error>> {
    let producer_config = ProducerConfig::new(KAFKA_BROKERS, ORDERS_TOPIC)
        .acks(AckMode::All);
    ...
    loop {
        ...
        match producer.send(
            ...
            Ok(_) => println!("📦 Generated order: {} - {} ${:.2} for {}", 
