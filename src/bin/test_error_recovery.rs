...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for i in 1..=2 {
        let result = circuit_breaker.call(async {
            Ok::<_, SqlError>(format!("Success {}", i))
        }).await;
    ...
    for i in 1..=3 {
        ...
        println!("   • Failure {}: Circuit state = {:?}", i, circuit_breaker.state().await);
    }

    // Test circuit open behavior
    println!("\nTesting circuit open behavior:");
    let result = circuit_breaker.call(async {
        Ok::<_, SqlError>("Should be blocked".to_string())
    }).await;
    match result {
        Err(RecoveryError::CircuitOpen { service, retry_after, .. }) => {
            println!("   ✅ Circuit correctly blocked request for '{}', retry in {:?}", service, retry_after);
        }
    ...
    println!("   • Circuit opened: {} times", cb_metrics.circuit_opened_count);
    ...
    match result {
        Ok(success_msg) => {
            println!("   ✅ Operation succeeded after {} attempts in {:?}", final_attempt_count, total_time);
    ...
    let exhaustion_result = retry_policy.execute(|| {
        Box::pin(async {
    ...
    match exhaustion_result {
        Err(RecoveryError::RetryExhausted { attempts, .. }) => {
            println!("   ✅ Retry correctly exhausted after {} attempts", attempts);
        }
    ...
    for i in 1..=3 {
        ...
        let failed_msg = FailedMessage {
            ...
            error_details: format!("Schema validation failed: missing field 'customer_id' in message {}", i),
    ...
    println!("   • Current queue size: {}", dlq_metrics.current_queue_size);

    // Retrieve messages for processing
    let failed_messages = dlq.dequeue(2).await?;
    println!("   ✅ Retrieved {} messages for manual processing:", failed_messages.len());
    ...
    // Register components
    health_monitor.register_component("database".to_string()).await;
    health_monitor.register_component("kafka_cluster".to_string()).await;
    health_monitor.register_component("schema_registry".to_string()).await;
    println!("   • Registered 3 components for monitoring");

    // Update component health statuses
    let mut db_details = HashMap::new();
    db_details.insert("connection_pool".to_string(), "10/20 active connections".to_string());
    db_details.insert("response_time_ms".to_string(), "45".to_string());
    
    health_monitor.update_health("database", HealthStatus::Healthy, db_details).await;
    ...
    health_monitor.update_health("kafka_cluster", HealthStatus::Healthy, kafka_details).await;

    let mut schema_details = HashMap::new();
    schema_details.insert("registry_url".to_string(), "http://localhost:8081".to_string());
    schema_details.insert("schema_count".to_string(), "25".to_string());
    
    health_monitor.update_health("schema_registry", HealthStatus::Degraded, schema_details).await;
    ...
    println!("   • Components healthy: {}", health_metrics.components_healthy);
    println!("   • Components unhealthy: {}", health_metrics.components_unhealthy);
    ...
    for i in 1..=2 {
        let result = circuit_breaker.call(async move {
            Ok::<_, SqlError>(format!("Recovery success {}", i))
        }).await;
    ...
    println!("   • Successful requests: {}", final_cb_metrics.successful_requests);
    ...
    println!("     - Half-opened: {}", final_cb_metrics.circuit_half_opened_count);
    println!("     - Closed: {}", final_cb_metrics.circuit_closed_count);
    println!("   • Average response time: {:.2}ms", final_cb_metrics.avg_response_time_ms);
    ...
    println!("   • DLQ maintenance: removed {} expired messages", dlq_maintenance);
