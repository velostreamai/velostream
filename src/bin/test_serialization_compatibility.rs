...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    record.insert("total".to_string(), FieldValue::from_financial_f64(12345.67, 2));
    record.insert("name".to_string(), FieldValue::String("AAPL Trade".to_string()));
    
    println!("Original Record:");
    for (key, value) in &record {
        println!("  {}: {} (type: {})", key, value.to_display_string(), value.type_name());
    }
    ...
    match parsed_json {
        serde_json::Value::Object(obj) => {
            for (key, value) in &obj {
                match value {
                    serde_json::Value::String(s) => println!("  {}: \"{}\" (string - can be parsed as decimal)", key, s),
    ...
    if let (Some(price), Some(quantity)) = (restored.get("price"), restored.get("quantity")) {
        match price.multiply(quantity) {
            Ok(calculated_total) => {
                ...
                println!("  Price × Quantity = {}", calculated_total.to_display_string());
            }
