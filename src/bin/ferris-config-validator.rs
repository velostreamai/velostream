#!/usr/bin/env cargo
//! FerrisStreams Configuration Validator CLI
//!
...
fn main() {
    let matches = Command::new("ferris-config-validator")
        .version("1.0.0")
        .about("FerrisStreams Configuration Validator")
        .long_about("Validates FerrisStreams configuration files against the schema and performs comprehensive validation checks.")
        .arg(
            Arg::new("config")
        ...
        .arg(
            Arg::new("schema")
                ...
                .default_value("ferrisstreams-config.schema.json"),
        )
        .arg(
            Arg::new("verbose")
                .long("verbose")
    ...
    if verbose {
        println!("🔧 FerrisStreams Configuration Validator");
