...
async fn test_file_user_properties_not_overridden() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let _writer = file_sink.create_writer_with_batch_config(batch_config).await?;
    ...
    let _writer_mem = file_sink_mem.create_writer_with_batch_config(memory_config).await?;
    // Log should show: 
    ...
    let _writer_ll = file_sink_ll.create_writer_with_batch_config(low_latency_config).await?;
    ...
    let _writer_default = file_sink_default.create_writer_with_batch_config(time_window_config).await?;
    ...
    let _writer_disabled = file_sink_disabled.create_writer_with_batch_config(disabled_config).await?;
