use ferrisstreams::ferris::sql::schema::{
    BackendConfig, SchemaRegistryBackendFactory, SchemaReference
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let schema_id = backend.register_schema("users-value", test_schema, vec![]).await?;
    ...
    println!("🔍 Retrieved schema by ID {}: subject='{}', version={}", 
        retrieved_schema.id, 
        retrieved_schema.subject, 
        retrieved_schema.version
    );
    
    // Test latest schema retrieval
    let latest_schema = backend.get_latest_schema("users-value").await?;
    println!("📋 Latest schema: ID={}, version={}", latest_schema.id, latest_schema.version);
    ...
    let address_id = backend.register_schema("addresses-value", address_schema, vec![]).await?;
    ...
    let user_ref_id = backend.register_schema(
        "users-with-address-value", 
        user_with_address_schema, 
        vec![address_ref]
    ).await?;
    println!("✅ User with address schema registered with ID: {}", user_ref_id);
    ...
    let is_compatible = backend.check_compatibility("users-value", modified_schema).await?;
    println!("🔄 Schema compatibility check: {}", if is_compatible { "✅ Compatible" } else { "❌ Incompatible" });
    
    // Test metadata
    let metadata = backend.metadata();
    println!("🔧 Backend metadata: type={}, version={}", metadata.backend_type, metadata.version);
    println!("   Features: {:?}", metadata.supported_features);
    println!("   Supports references: {}", metadata.capabilities.supports_references);
    
    // Register another version of the user schema
    println!("📝 Registering version 2 of user schema");
    let user_v2_id = backend.register_schema("users-value", modified_schema, vec![]).await?;
    ...
    println!("📋 Latest schema after v2: ID={}, version={}", latest_after_v2.id, latest_after_v2.version);
    
    // Test specific version retrieval
    let v1_schema = backend.get_schema_version("users-value", 1).await?;
    println!("📋 Version 1 schema: ID={}, version={}", v1_schema.id, v1_schema.version);
    
    let v2_schema = backend.get_schema_version("users-value", 2).await?;
    println!("📋 Version 2 schema: ID={}, version={}", v2_schema.id, v2_schema.version);
