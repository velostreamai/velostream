...
use ferrisstreams::ferris::config::{HierarchicalSchemaRegistry, ConfigSchemaProvider};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📊 Schema includes {} configuration types", count_schema_types(&json_schema));
...
fn generate_comprehensive_json_schema(registry: &HierarchicalSchemaRegistry) -> Result<Value, Box<dyn std::error::Error>> {
    let mut schema = json!({
    ...
    properties.insert("sources".to_string(), json!({
        "type": "object",
    ...
    properties.insert("sinks".to_string(), json!({
        "type": "object", 
    ...
    properties.insert("global".to_string(), json!({
        "type": "object",
    ...
    properties.insert("extends".to_string(), json!({
        "oneOf": [
            {
                "type": "string",
...
fn generate_ide_configurations(_schema: &Value) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fs::write(".vscode/ferris-schema-settings.json", serde_json::to_string_pretty(&vscode_settings)?)?;
    
    println!("📝 Generated: .vscode/ferris-schema-settings.json");
    
    // Generate example configuration with schema reference
    let example_config = format!(r#"# yaml-language-server: $schema=./ferrisstreams-config.schema.json
