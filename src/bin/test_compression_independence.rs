...
async fn test_kafka_explicit_compression() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let _writer = kafka_sink.create_writer_with_batch_config(fixed_size_config).await?;
    ...
    let _writer = kafka_sink_none.create_writer_with_batch_config(memory_config).await?;
...
async fn test_file_explicit_compression() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let _writer = file_sink.create_writer_with_batch_config(memory_config).await?;
    ...
    let _writer = file_sink_auto.create_writer_with_batch_config(memory_config_auto).await?;
