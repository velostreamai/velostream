...
use crate::ferris::sql::execution::config::PrometheusConfig;
use prometheus::{
    ...
    fn new(registry: &Registry, config: &PrometheusConfig) -> Result<Self, SqlError> {
        ...
        let query_duration = if config.enable_histograms {
            register_histogram_with_registry!(
                HistogramOpts::new(
                    "ferris_sql_query_duration_seconds",
        ...
        let records_processed = register_int_counter_with_registry!(
            Opts::new(
                "ferris_sql_records_processed_total",
...
impl StreamingMetrics {
    fn new(registry: &Registry, config: &PrometheusConfig) -> Result<Self, SqlError> {
        let operations_total = register_int_counter_with_registry!(
            Opts::new(
                "ferris_streaming_operations_total",
        ...
        let operation_duration = if config.enable_histograms {
            register_histogram_with_registry!(
                HistogramOpts::new(
                    "ferris_streaming_duration_seconds",
        ...
        let throughput = register_gauge_with_registry!(
            Opts::new(
                "ferris_streaming_throughput_rps",
        ...
        let records_streamed = register_int_counter_with_registry!(
            Opts::new(
                "ferris_streaming_records_total",
...
impl SystemMetrics {
    fn new(registry: &Registry, _config: &PrometheusConfig) -> Result<Self, SqlError> {
        let cpu_usage = register_gauge_with_registry!(
            Opts::new("ferris_cpu_usage_percent", "Current CPU usage percentage"),
            registry
        )
        .map_err(|e| SqlError::ConfigurationError {
        ...
        let memory_usage = register_int_gauge_with_registry!(
            Opts::new("ferris_memory_usage_bytes", "Current memory usage in bytes"),
            registry
        )
        .map_err(|e| SqlError::ConfigurationError {
        ...
        let active_connections = register_int_gauge_with_registry!(
            Opts::new("ferris_active_connections", "Number of active connections"),
            registry
        )
        .map_err(|e| SqlError::ConfigurationError {
...
mod tests {
    ...
    async fn test_metrics_text_export() {
        ...
        assert!(text.contains("ferris_sql_queries_total"));
        assert!(text.contains("ferris_sql_query_duration_seconds"));
    }
