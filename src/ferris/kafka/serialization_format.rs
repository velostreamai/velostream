...
use crate::ferris::kafka::serialization::{
    JsonSerializer, SerializationError, Serializer,
};
...
impl SerializationConfig {
    ...
    pub fn from_sql_params(params: &HashMap<String, String>) -> Result<Self, SerializationError> {
        ...
        if let Some(key_serializer) = params.get("key.serializer")
            .or_else(|| params.get("key_serializer")) 
        {
            let format: SerializationFormat = key_serializer.parse()?;
            
            // Configure Avro-specific parameters
            #[cfg(feature = "avro")]
            if let SerializationFormat::Avro { schema_registry_url, subject } = &mut format {
        ...
        if let Some(value_serializer) = params.get("value.serializer")
            .or_else(|| params.get("value_serializer"))
        {
            let format: SerializationFormat = value_serializer.parse()?;
    ...
    pub fn value_format(&self) -> SerializationFormat {
        self.value_format.clone().unwrap_or(SerializationFormat::Json)
    }
...
impl SerializationFactory {
        ...
        T: Serialize + for<'de> Deserialize<'de> + 'static,
    {
        match format {
            ...
            #[cfg(feature = "avro")]
            SerializationFormat::Avro { schema_registry_url, subject } => {
            ...
            SerializationFormat::Protobuf { message_type } => {
                ...
                // in a later step
                Err(SerializationError::FeatureNotEnabled(
                    format!("Protobuf serialization not yet implemented. Message type: {}", message_type)
                ))
            }
...
mod tests {
    ...
    fn test_serialization_config_from_sql_params() {
        ...
        assert_eq!(config.custom_properties.get("custom.param"), Some(&"custom_value".to_string()));
    }
