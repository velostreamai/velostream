use crate::ferris::kafka::common_config::CommonKafkaConfig;
...
pub mod presets {
    ...
    pub fn apply_high_throughput_common(config: &mut CommonKafkaConfig) {
        ...
        config.custom_config.insert("socket.send.buffer.bytes".to_string(), "131072".to_string()); // 128KB
        config.custom_config.insert("socket.receive.buffer.bytes".to_string(), "131072".to_string()); // 128KB
    }
    ...
    pub fn apply_low_latency_common(config: &mut CommonKafkaConfig) {
        ...
        config.custom_config.insert("socket.send.buffer.bytes".to_string(), "32768".to_string()); // 32KB
        config.custom_config.insert("socket.receive.buffer.bytes".to_string(), "32768".to_string()); // 32KB
    }
    ...
    pub fn apply_max_durability_common(config: &mut CommonKafkaConfig) {
        ...
        config.custom_config.insert("socket.send.buffer.bytes".to_string(), "262144".to_string()); // 256KB
        config.custom_config.insert("socket.receive.buffer.bytes".to_string(), "262144".to_string()); // 256KB
    }
    ...
    pub fn apply_development_common(config: &mut CommonKafkaConfig) {
        ...
        config.custom_config.insert("api.version.request".to_string(), "true".to_string());
        config.custom_config.insert("log.connection.close".to_string(), "false".to_string());
    }
    ...
    pub fn apply_streaming_common(config: &mut CommonKafkaConfig) {
        ...
        config.custom_config.insert("socket.send.buffer.bytes".to_string(), "65536".to_string()); // 64KB
        config.custom_config.insert("socket.receive.buffer.bytes".to_string(), "65536".to_string()); // 64KB
    }
    ...
    pub fn apply_batch_processing_common(config: &mut CommonKafkaConfig) {
        ...
        config.custom_config.insert("socket.send.buffer.bytes".to_string(), "524288".to_string()); // 512KB
        config.custom_config.insert("socket.receive.buffer.bytes".to_string(), "524288".to_string()); // 512KB
    }
}
...
pub trait PerformancePresets {
    ...
    fn streaming(self) -> Self where Self: Sized {
        self // Default implementation does nothing
    }
    
    /// Apply batch processing preset (if applicable)
    fn batch_processing(self) -> Self where Self: Sized {
        self // Default implementation does nothing
    }
}
...
pub mod utils {
    ...
    pub fn get_preset_description(preset_name: &str) -> Option<&'static str> {
        match preset_name {
            "high_throughput" => Some("Optimized for maximum message throughput with larger batches and buffers"),
            "low_latency" => Some("Optimized for minimal latency with immediate processing and small buffers"),
            "max_durability" => Some("Optimized for data safety with conservative timeouts and reliability features"),
            "development" => Some("Development-friendly settings with reasonable timeouts and debug features"),
            "streaming" => Some("Balanced settings for continuous stream processing workloads"),
            "batch_processing" => Some("Optimized for periodic batch processing with large buffers and extended timeouts"),
            _ => None,
        }
    }
...
mod tests {
    ...
    fn test_high_throughput_preset() {
        ...
        assert!(config.custom_config.contains_key("socket.send.buffer.bytes"));
        assert_eq!(config.custom_config.get("socket.send.buffer.bytes"), Some(&"131072".to_string()));
    }
    ...
    fn test_low_latency_preset() {
        ...
        assert!(config.custom_config.contains_key("socket.send.buffer.bytes"));
        assert_eq!(config.custom_config.get("socket.send.buffer.bytes"), Some(&"32768".to_string()));
    }
    ...
    fn test_max_durability_preset() {
        ...
        assert!(config.custom_config.contains_key("socket.send.buffer.bytes"));
        assert_eq!(config.custom_config.get("socket.send.buffer.bytes"), Some(&"262144".to_string()));
    }
