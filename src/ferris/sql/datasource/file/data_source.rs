...
impl FileDataSource {
    ...
    async fn validate_path(&self, path: &str) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        if path.contains('*') || path.contains('?') {
            ...
            if !path_obj.exists() {
                return Err(Box::new(FileDataSourceError::FileNotFound(path.to_string())));
            }
    ...
    async fn infer_schema(&self, config: &FileSourceConfig) -> Result<Schema, Box<dyn Error + Send + Sync>> {
        match config.format {
            FileFormat::Csv | FileFormat::CsvNoHeader => {
                self.infer_csv_schema(config).await
            }
            FileFormat::JsonLines | FileFormat::Json => {
                self.infer_json_schema(config).await
            }
        }
    }
    
    /// Infer schema from CSV file
    async fn infer_csv_schema(&self, config: &FileSourceConfig) -> Result<Schema, Box<dyn Error + Send + Sync>> {
        use std::fs::File;
    ...
    async fn infer_json_schema(&self, config: &FileSourceConfig) -> Result<Schema, Box<dyn Error + Send + Sync>> {
        use std::fs::File;
        ...
        match config.format {
            ...
            FileFormat::Json => {
                ...
                fields.push(FieldDefinition::new("data".to_string(), DataType::String, true));
            }
            _ => unreachable!(),
        }
...
impl DataSource for FileDataSource {
    async fn initialize(
        ...
        let file_config = match config {
            ...
            _ => return Err(Box::new(FileDataSourceError::UnsupportedFormat(
        ...
        file_config.validate()
            .map_err(|e| Box::new(FileDataSourceError::InvalidPath(e)) as Box<dyn Error + Send + Sync>)?;
        ...
        if file_config.watch_for_changes {
            ...
            watcher.watch(&file_config.path, file_config.polling_interval_ms).await?;
