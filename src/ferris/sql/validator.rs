...
use crate::ferris::sql::{
    ...
    query_analyzer::{DataSinkRequirement, DataSourceRequirement, DataSourceType, DataSinkType, QueryAnalyzer},
...
impl SqlValidator {
    ...
    fn validate_query(&self, query: &str, query_index: usize, start_line: usize, full_content: &str) -> QueryValidationResult {
    ...
    fn validate_source_configurations(&self, sources: &[DataSourceRequirement], result: &mut QueryValidationResult) {
        for source in sources {
            ...
            match &source.source_type {
                DataSourceType::Kafka => self.validate_kafka_source_config(properties, &source.name, result),
                DataSourceType::File => self.validate_file_source_config(properties, &source.name, result),
                _ => {
    ...
    fn validate_sink_configurations(&self, sinks: &[DataSinkRequirement], result: &mut QueryValidationResult) {
        for sink in sinks {
            ...
            match &sink.sink_type {
                DataSinkType::Kafka => self.validate_kafka_sink_config(properties, &sink.name, result),
                DataSinkType::File => self.validate_file_sink_config(properties, &sink.name, result),
                _ => {
                    result.warnings.push(ValidationError {
    ...
    fn validate_kafka_source_config(&self, properties: &HashMap<String, String>, name: &str, result: &mut QueryValidationResult) {
        ...
        if !missing_keys.is_empty() {
            result.missing_source_configs.push(ConfigurationIssue {
                ...
                missing_keys,
                has_batch_config: properties.contains_key("batch.size") || properties.contains_key("max.poll.records"),
    ...
    fn validate_kafka_sink_config(&self, properties: &HashMap<String, String>, name: &str, result: &mut QueryValidationResult) {
        ...
        if !missing_keys.is_empty() {
            result.missing_sink_configs.push(ConfigurationIssue {
                ...
                required_keys: required_keys.iter().map(|s| s.to_string()).collect(),
                missing_keys,
                has_batch_config: properties.contains_key("batch.size") || properties.contains_key("linger.ms"),
                has_failure_strategy: properties.contains_key("failure_strategy"),
            });
            result.is_valid = false;
        }
    ...
    fn analyze_configuration_completeness(&self, result: &mut ApplicationValidationResult) {
        ...
        for query_result in &result.query_results {
            for source_issue in &query_result.missing_source_configs {
                result.configuration_summary.missing_configurations.push(
                    format!("Source '{}': missing {}", source_issue.name, source_issue.missing_keys.join(", "))
                );
            }
            for sink_issue in &query_result.missing_sink_configs {
                result.configuration_summary.missing_configurations.push(
                    format!("Sink '{}': missing {}", sink_issue.name, sink_issue.missing_keys.join(", "))
                );
            }
        }
    }
    ...
    fn extract_error_location(&self, error: &crate::ferris::sql::error::SqlError, query: &str, start_line: usize) -> (usize, usize, String) {
    ...
    fn generate_error_context(&self, query: &str, position: usize, line_offset: usize, column: usize) -> String {
        ...
        context.push_str(&format!("  {}{}^ Error here", 
