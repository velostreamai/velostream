...
use super::registry_backend::{SchemaRegistryBackend, BackendConfig, SchemaRegistryBackendFactory};
use super::enhanced_cache::EnhancedSchemaCache;
use super::reference_resolver::SchemaReferenceResolver;
use super::registry_client::{CachedSchema, SchemaReference, ResolvedSchema, DependencyGraph};
...
impl UnifiedSchemaRegistryClient {
    ...
    pub fn new(backend_config: BackendConfig) -> SchemaResult<Self> {
        ...
        // Create resolver with backend reference (avoiding circular dependency)
        // For now, we create a stub resolver that will be properly integrated later
        let resolver = Arc::new(SchemaReferenceResolver::new(
            Arc::new(crate::ferris::sql::schema::registry_client::SchemaRegistryClient::new("unified-client".to_string()))
        ));
    ...
    pub fn with_config(backend_config: BackendConfig, client_config: UnifiedClientConfig) -> SchemaResult<Self> {
    ...
    pub async fn get_schema_version(&self, subject: &str, version: i32) -> SchemaResult<CachedSchema> {
    ...
    pub async fn register_schema(&self, subject: &str, schema: &str, references: Vec<SchemaReference>) -> SchemaResult<u32> {
        self.backend.register_schema(subject, schema, references).await
    }
    ...
    pub async fn resolve_schema_with_references(&self, id: u32) -> SchemaResult<ResolvedSchema> {
        ...
        // Use the resolver for complex reference resolution
        // TODO: Integrate properly with the resolver
        self.resolver.resolve_references(id).await
    }
...
impl UnifiedClientBuilder {
    ...
    pub fn confluent_with_auth(mut self, base_url: &str, auth: super::registry_backend::ConfluentAuth) -> Self {
        self.backend_config = Some(BackendConfig::Confluent {
            base_url: base_url.to_string(),
            auth,
            timeout_seconds: 30,
...
mod tests {
    ...
    fn test_backend_auto_detection() {
        ...
        let pulsar_config = SchemaRegistryBackendFactory::auto_detect("http://localhost:8080/admin/v2/schemas");
        matches!(pulsar_config, BackendConfig::Pulsar { .. });
        
        // Test AWS MSK detection
        let aws_config = SchemaRegistryBackendFactory::auto_detect("arn:aws:kafka:us-east-1:123456789012:cluster/my-cluster");
