/// Unified configuration management system for FerrisStreams
/// 
...
impl StructConfigSuggestor<super::super::file::config::FileSinkConfig> {
    ...
    pub fn suggest_compression(&mut self, compression: Option<super::super::file::config::CompressionType>) {
...
impl BatchConfigApplicator {
    ...
    pub fn apply_file_sink_strategy(
        ...
        match strategy {
            ...
            BatchStrategy::LowLatency { eager_processing, .. } => {
                if *eager_processing {
    ...
    fn apply_fixed_size_kafka(config: &mut HashMap<String, String>, size: usize, max_batch_size: usize) {
...
impl ConfigLogger {
    ...
    pub fn log_kafka_producer_config(
        ...
        info!("  - batch.size: {} {}", 
              config.get("batch.size").unwrap_or(&defaults::KAFKA_BATCH_SIZE.to_string()),
              if config.is_user_set("batch.size") { "(user)" } else { "(suggested)" });
        info!("  - linger.ms: {} {}", 
              config.get("linger.ms").unwrap_or(&defaults::KAFKA_LINGER_MS.to_string()),
              if config.is_user_set("linger.ms") { "(user)" } else { "(suggested)" });
        info!("  - compression.type: {} {}", 
              config.get("compression.type").unwrap_or(&defaults::KAFKA_COMPRESSION_TYPE.to_string()),
              if config.is_user_set("compression.type") { "(user)" } else { "(suggested)" });
        info!("  - acks: {} {}", 
              config.get("acks").unwrap_or(&defaults::KAFKA_ACKS.to_string()),
              if config.is_user_set("acks") { "(user)" } else { "(suggested)" });
        info!("  - retries: {} {}", 
              config.get("retries").unwrap_or(&defaults::KAFKA_RETRIES.to_string()),
              if config.is_user_set("retries") { "(user)" } else { "(suggested)" });
              
        if let Some(buffer_memory) = config.get("queue.buffering.max.kbytes") {
            info!("  - queue.buffering.max.kbytes: {}KB {}", 
                  buffer_memory,
                  if config.is_user_set("queue.buffering.max.kbytes") { "(user)" } else { "(suggested)" });
        }
        info!("  - topic: {}", topic);
    ...
    pub fn log_file_sink_config(
        ...
        info!("  - buffer_size_bytes: {} {}", 
              config.buffer_size_bytes,
              if config_wrapper.is_user_set("buffer_size_bytes") { "(user)" } else { "(suggested)" });
        info!("  - compression: {:?} {}", 
              config.compression,
              if config_wrapper.is_user_set("compression") { "(user)" } else { "(suggested)" });
