...
impl std::fmt::Display for SerializationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            SerializationError::FormatConversionFailed(msg) => write!(f, "Format conversion failed: {}", msg),
    ...
    match field_value {
        ...
        FieldValue::Float(f) => {
            serde_json::Number::from_f64(*f)
                ...
                .ok_or_else(|| SerializationError::FormatConversionFailed(format!("Invalid float: {}", f)))
        },
...
fn internal_to_field_value(internal_value: &InternalValue) -> Result<FieldValue, SerializationError> {
...
impl SerializationFormatFactory {
    ...
    pub fn create_format(format_name: &str) -> Result<Box<dyn SerializationFormat>, SerializationError> {
        match format_name.to_lowercase().as_str() {
            "json" => Ok(Box::new(JsonFormat)),
            _ => Err(SerializationError::UnsupportedType(format!("Unknown format: {}", format_name)))
        }
    }
