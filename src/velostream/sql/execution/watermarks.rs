    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::BoundedOutOfOrderness {
                ...
                .debug_struct("BoundedOutOfOrderness")
...
impl std::fmt::Debug for LateDataStrategy {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::DeadLetterQueue { queue_name } => f
                .debug_struct("DeadLetterQueue")
                .field("queue_name", queue_name)
                .finish(),
            Self::UpdatePreviousWindow { grace_period } => f
                .debug_struct("UpdatePreviousWindow")
...
impl WatermarkManager {
    ...
    pub fn get_source_watermark(&self, source_id: &str) -> Option<DateTime<Utc>> {
        self.source_watermarks.read().ok()?.get(source_id).copied()
    }
    ...
    fn update_global_watermark(&self) {
        if let (Ok(sources), Ok(mut global)) =
            (self.source_watermarks.read(), self.global_watermark.write())
        {
