...
impl ProfilingConfig {
    ...
    pub fn production() -> Self {
        Self {
            ...
            output_directory: "/var/log/velo-profiling".to_string(),
            sample_rate_hz: 50,
...
mod tests {
    ...
    fn test_tracing_config_development() {
        ...
        assert_eq!(config.service_name, "velo-streams-dev");
    ...
    fn test_profiling_config_production() {
        ...
        assert!(config.enable_cpu_profiling);
        assert!(!config.enable_memory_profiling);
        assert_eq!(config.output_directory, "/var/log/velo-profiling");
