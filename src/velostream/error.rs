...
use crate::velostream::kafka::kafka_error::KafkaClientError;
...
/// Type alias for Results using VeloError
pub type VeloResult<T> = Result<T, VeloError>;

/// Convert from standard Box<dyn std::error::Error> to VeloError
impl From<Box<dyn std::error::Error + Send + Sync>> for VeloError {
    fn from(err: Box<dyn std::error::Error + Send + Sync>) -> Self {
        VeloError::Application {
            message: err.to_string(),
        }
    }
}

impl From<Box<dyn std::error::Error>> for VeloError {
