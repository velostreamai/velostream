...
async fn test_multiple_user_workflow() {
    ...
    let users = vec![
        User { id: 1, name: "Alice".to_string(), email: Some("alice@test.com".to_string()) },
        User { id: 2, name: "Bob".to_string(), email: None },
        User { id: 3, name: "Charlie".to_string(), email: Some("charlie@test.com".to_string()) },
        User { id: 4, name: "Diana".to_string(), email: Some("diana@test.com".to_string()) },
        User { id: 5, name: "Eve".to_string(), email: None },
    ];
...
async fn test_cross_topic_messaging() {
    ...
    product_consumer.subscribe(&[&product_topic]).expect("Failed to subscribe to products");  
    order_consumer.subscribe(&[&order_topic]).expect("Failed to subscribe to orders");

    // Send different message types
    let user = User { id: 100, name: "Cross User".to_string(), email: Some("cross@test.com".to_string()) };
    let product = Product { 
    ...
    user_consumer.commit().expect("Failed to commit user consumer");
    product_consumer.commit().expect("Failed to commit product consumer");
    order_consumer.commit().expect("Failed to commit order consumer");
...
async fn test_high_throughput_scenario() {
    ...
    println!("  Received {} messages in {:?}", received_count, consume_duration);
    println!("  Send rate: {:.1} msg/sec", message_count as f64 / send_duration.as_secs_f64());
    
    // We should receive at least 80% of messages in reasonable time
    let success_rate = received_count as f64 / message_count as f64;
    assert!(success_rate > 0.8, "Should receive most messages, got {:.1}%", success_rate * 100.0);
    assert!(send_duration < Duration::from_secs(10), "Sending should be reasonably fast");
}
...
async fn test_complex_enum_serialization() {
    ...
    let statuses: std::collections::HashSet<_> = received_orders.iter().map(|o| &o.status).collect();
