/*!
# Transactional Multi-Source Processor Sink Write Verification Test

This test verifies that the transactional multi-source processor actually writes processed records to sinks
within transactions, addressing the bug where records were processed but never written.
*/
...
use velostream::velostream::server::processors::{
    FailureStrategy, JobProcessingConfig, TransactionalJobProcessor,
};
use velostream::velostream::sql::execution::{FieldValue, StreamRecord};
use velostream::velostream::sql::{StreamExecutionEngine, StreamingSqlParser};

/// Mock DataReader for testing (non-transactional)
#[derive(Debug)]
...
/// Mock DataWriter that tracks written records using Arc for sharing (non-transactional)
#[derive(Debug, Clone)]
...
impl DataWriter for MockDataWriter {
    ...
    async fn flush(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        println!(
            "MockDataWriter '{}' flushed {} records (transactional test)",
            self.name, count
        );
        Ok(())
    }
...
#[tokio::test]
async fn test_transactional_multi_source_processor_writes_to_sinks() {
    // This test verifies the fix for the bug where process_multi_source_transactional_batch()
    ...
    println!("Records written to sink (transactional): {}", written_count);

    assert!(
        written_count > 0,
        "REGRESSION: Records were processed (stats.records_processed={}) but NOT written to sink! \
         This is the bug we fixed - transactional processor must write output to sinks within transactions.",
        stats.records_processed
    );

    // All processed records should be written (for simple passthrough queries)
    assert_eq!(
        written_count, stats.records_processed as usize,
        "All processed records should be written to sink in transactional mode"
    );

    // Verify records are SQL OUTPUT, not input passthrough
    let written_records = writer_clone.get_written_records();
    println!(
        "Verifying {} written records are SQL output from transactional processor",
        written_records.len()
    );
    ...
    println!(
        "✅ All {} records verified as SQL query output from transactional processor",
        written_records.len()
    );
}
