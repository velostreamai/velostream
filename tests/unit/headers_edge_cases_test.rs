...
fn test_headers_with_binary_data() {
    ...
    assert_eq!(headers.get("binary").unwrap(), String::from_utf8_lossy(&binary_data));
    assert_eq!(headers.get("null-bytes").unwrap(), String::from_utf8_lossy(&null_bytes));
    assert_eq!(headers.get("mixed").unwrap(), String::from_utf8_lossy(&mixed));
    ...
    for (key, value) in headers.iter() {
        if key == "binary" {
            found_binary = true;
            assert_eq!(value, &Some(String::from_utf8_lossy(&binary_data).to_string()));
        }
    }
    assert!(found_binary);
}
...
fn test_headers_size_limits() {
    ...
    assert_eq!(headers.get("small").map(ToString::to_string), Some(small_value.clone()));
    assert_eq!(headers.get("medium").map(ToString::to_string), Some(medium_value.clone()));
    assert_eq!(headers.get("large").map(ToString::to_string), Some(large_value.clone()));

    // Verify iteration works with large values
    for (key, value) in headers.iter() {
        match key.as_str() {
            "small" => assert_eq!(value.as_ref().map(|s| s.to_string()), Some(small_value.clone())),
...
fn test_headers_special_characters() {
    ...
    assert_eq!(headers.get("symbols"), Some("!@#$%^&*()_+-={}[]|\\:;\"'<>?,./"));
...
fn test_headers_null_values() {
    ...
    assert_eq!(headers.get_optional("regular-header"), Some(&Some("regular-value".to_string())));
...
fn test_headers_capacity_and_performance() {
    ...
    for i in 0..50 {
        headers_with_capacity = headers_with_capacity.insert(&format!("key{}", i), &format!("value{}", i));
    }
    ...
    for i in 0..50 {
        assert_eq!(headers_with_capacity.get(&format!("key{}", i)), Some(format!("value{}", i).as_str()));
    }
}
