...
fn test_comprehensive_error_source_chain_preservation() {
    ...
    assert!(contextual_error.source().is_some(), "Top-level error should have source");
    ...
    match kafka_source.downcast_ref::<KafkaClientError>() {
        Some(KafkaClientError::SerializationError(ser_err)) => {
            ...
            assert!(ser_err.source().is_some(), "SerializationError should have source");
        ...
        _ => panic!("Expected SerializationError in chain, got: {:?}", kafka_source),
    }
    
    // Test error display includes comprehensive context
    let error_string = contextual_error.to_string();
    assert!(error_string.contains("send_message"), "Should contain operation");
    assert!(error_string.contains("high-priority-orders"), "Should contain topic");
    assert!(error_string.contains("correlation_id"), "Should contain correlation ID");
    
    // Verify all metadata is accessible
    assert_eq!(contextual_error.context.metadata.get("key"), Some(&"order-12345".to_string()));
    assert_eq!(contextual_error.context.metadata.get("partition"), Some(&"3".to_string()));
    assert_eq!(contextual_error.context.metadata.get("broker"), Some(&"broker-2.kafka.internal:9092".to_string()));
    assert_eq!(contextual_error.context.metadata.get("retry_attempt"), Some(&"2".to_string()));
...
fn test_operational_error_metadata_tracking() {
    ...
    for (i, (operation, topic, component)) in operations.iter().enumerate() {
        let context = ErrorContext::new(operation, topic)
            ...
            .with_metadata("availability_zone", &format!("us-west-2{}", char::from(b'a' + i as u8)))
    ...
    for (i, context) in operation_contexts.iter().enumerate() {
        ...
        assert_eq!(context.metadata.get("component"), Some(&operations[i].2.to_string()));
        assert_eq!(context.metadata.get("pipeline_stage"), Some(&format!("stage_{}", i + 1)));
        assert_eq!(context.metadata.get("request_id"), Some(&"req_abc123_xyz789".to_string()));
        ...
        if i > 0 {
            assert!(context.timestamp >= operation_contexts[i-1].timestamp,
    ...
    assert_eq!(error.context.metadata.get("component"), Some(&"enricher-service-2".to_string()));
    assert_eq!(error.context.metadata.get("pipeline_stage"), Some(&"stage_3".to_string()));
    ...
    let error_report = format!(
        ...
        error.context.topic,
        error.context.metadata.get("component").unwrap_or(&"unknown".to_string()),
        error.context.metadata.get("pipeline_stage").unwrap_or(&"unknown".to_string()),
        error.context.metadata.get("instance_id").unwrap_or(&"unknown".to_string()),
        error.context.correlation_id,
        error.context.metadata.get("request_id").unwrap_or(&"unknown".to_string()),
        error.context.metadata.get("environment").unwrap_or(&"unknown".to_string()),
        error.context.metadata.get("region").unwrap_or(&"unknown".to_string()),
        error.context.metadata.get("availability_zone").unwrap_or(&"unknown".to_string()),
        error.context.metadata.get("cpu_usage").unwrap_or(&"unknown".to_string()),
        error.context.metadata.get("memory_usage").unwrap_or(&"unknown".to_string()),
        error.inner
    );
...
fn test_error_severity_categorization() {
    ...
    assert_eq!(categorize_error(&kafka_serialization_error), ErrorSeverity::Critical);
}
...
impl ErrorMetrics {
    ...
    pub fn record_error(&mut self, error: &ContextualKafkaError) {
        // Count by operation
        let operation_count = self.total_errors.entry(error.context.operation.clone()).or_insert(0);
        *operation_count += 1;
...
fn test_error_metrics_collection() {
    ...
    assert_eq!(metrics.errors_by_severity.get(&ErrorSeverity::Warning), Some(&2)); // Timeouts
    assert_eq!(metrics.errors_by_severity.get(&ErrorSeverity::Info), Some(&1)); // NoMessage
    ...
    assert!(error_rate > 0.0, "Should have positive error rate for recent errors");
}
...
fn test_correlation_id_tracking() {
    ...
    let operations: Vec<&str> = contexts.iter()
        .map(|c| c.operation.as_str())
        .collect();
    
    assert_eq!(operations, vec!["validate_message", "send_message", "confirm_delivery"]);
    
    println!("✅ Correlation ID {} tracked across operations: {:?}", 
...
fn test_error_aggregation() {
    ...
    for topic in &topics {
        for operation in &operations {
            let context = ErrorContext::new(operation, topic)
                .with_metadata("batch_id", "batch-123");
            ...
            error_buckets.entry(key).or_insert_with(Vec::new).push(error);
        }
    }
    ...
    for (key, errors) in &error_buckets {
        ...
        assert_eq!(error.context.metadata.get("batch_id"), Some(&"batch-123".to_string()));
    }
    
    println!("✅ Aggregated {} error types across {} topics and {} operations", 
             error_buckets.len(), topics.len(), operations.len());
}
...
/// Test circuit breaker integration with error context
#[test]
fn test_circuit_breaker_error_context() {
    #[derive(Debug, PartialEq)]
    enum CircuitState { Closed, Open, HalfOpen }
    
    let mut circuit_errors: Vec<ContextualKafkaError> = Vec::new();
    let states = vec![CircuitState::Closed, CircuitState::Open, CircuitState::HalfOpen];
    
    for (i, state) in states.iter().enumerate() {
        let context = ErrorContext::new("circuit_breaker_send", "payment-processing")
            .with_metadata("circuit_state", &format!("{:?}", state))
            .with_metadata("failure_count", &(i * 5).to_string())
            .with_metadata("failure_threshold", "10")
            .with_metadata("timeout_duration", "30000")
            .with_metadata("last_failure_time", &chrono::Utc::now().to_rfc3339());
        
        let circuit_error = ContextualKafkaError {
            inner: KafkaClientError::Timeout,
            context,
        };
        
        circuit_errors.push(circuit_error);
    }
    
    // Verify circuit breaker state tracking
    assert_eq!(circuit_errors[0].context.metadata.get("circuit_state"), Some(&"Closed".to_string()));
    assert_eq!(circuit_errors[1].context.metadata.get("circuit_state"), Some(&"Open".to_string()));
    assert_eq!(circuit_errors[2].context.metadata.get("circuit_state"), Some(&"HalfOpen".to_string()));
    
    // Verify failure progression
    for (i, error) in circuit_errors.iter().enumerate() {
        assert_eq!(
            error.context.metadata.get("failure_count"),
            Some(&(i * 5).to_string())
        );
    }
    
    println!("✅ Circuit breaker error context integration validated");
}