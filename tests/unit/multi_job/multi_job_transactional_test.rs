...
use ferrisstreams::ferris::sql::{
    ...
    multi_job_transactional::TransactionalJobProcessor,
};
...
async fn test_transactional_processor_comprehensive_failure_scenarios() {
    ...
    run_comprehensive_failure_tests(&log_continue_processor, "TransactionalJobProcessor_LogAndContinue").await;
    ...
    test_source_read_failure_scenario(&retry_backoff_processor, "TransactionalJobProcessor_RetryWithBackoff").await;
    test_network_partition_scenario(&retry_backoff_processor, "TransactionalJobProcessor_RetryWithBackoff").await;
    test_partial_batch_failure_scenario(&retry_backoff_processor, "TransactionalJobProcessor_RetryWithBackoff").await;
    test_shutdown_signal_scenario(&retry_backoff_processor, "TransactionalJobProcessor_RetryWithBackoff").await;
    test_empty_batch_handling_scenario(&retry_backoff_processor, "TransactionalJobProcessor_RetryWithBackoff").await;
    ...
    run_comprehensive_failure_tests(&fail_batch_processor, "TransactionalJobProcessor_FailBatch").await;
}
...
async fn test_transactional_processor_rollback_behavior() {
    ...
    let reader = Box::new(AdvancedMockDataReader::new(test_batches).with_transaction_support()) as Box<dyn DataReader>;
    let writer = Box::new(AdvancedMockDataWriter::new()
        .with_transaction_support()
        .with_write_failure_on_batch(1)) as Box<dyn DataWriter>;
...
async fn test_transactional_processor_vs_simple_processor_behavior() {
    ...
    let transactional_reader = Box::new(AdvancedMockDataReader::new(test_batches.clone())
        .with_transaction_support()) as Box<dyn DataReader>;
    let transactional_writer = Box::new(AdvancedMockDataWriter::new()
        .with_transaction_support()
        .with_write_failure_on_batch(1)) as Box<dyn DataWriter>;
    ...
    let transactional_result = transactional_processor.process_job(
...
async fn test_transactional_processor_with_non_transactional_datasources() {
    ...
    assert!(result.is_ok(), "Should handle non-transactional datasources gracefully");
...
async fn test_transactional_processor_commit_ordering() {
    ...
    let reader = Box::new(AdvancedMockDataReader::new(test_batches).with_transaction_support()) as Box<dyn DataReader>;
    let writer = Box::new(AdvancedMockDataWriter::new().with_transaction_support()) as Box<dyn DataWriter>;
...
async fn test_transactional_processor_sink_commit_failure() {
    ...
    let writer = Box::new(AdvancedMockDataWriter::new()
        .with_transaction_support()
        .with_commit_failure_on_batch(0)) as Box<dyn DataWriter>;
