...
async fn test_job_execution_stats_metrics() {
    ...
    assert!(elapsed >= Duration::from_millis(100), "Should have elapsed time");
...
async fn test_retry_with_backoff_max_retries_exceeded() {
    ...
    let result = retry_with_backoff(
        || {
            attempt_count += 1;
            Box::pin(async move {
                Err(format!("Attempt {} failed", attempt_count).into()) as DataSourceResult<&str>
            })
        },
        2, // max_retries
        Duration::from_millis(1), // Very short backoff for testing
    ...
    assert_eq!(attempt_count, 3); // Original attempt + 2 retries
...
fn test_check_transaction_support_logging() {
    ...
    impl crate::ferris::datasource::DataReader for MockTransactionalReader {
        fn supports_transactions(&self) -> bool { true }
        // Other methods would have default implementations or panics for this test
    }
    
    // Mock reader that doesn't support transactions  
    struct MockNonTransactionalReader;
    impl crate::ferris::datasource::DataReader for MockNonTransactionalReader {
