...
mod file_datasource_tests {
    ...
    async fn test_csv_file_basic_reading() {
        ...
        assert_eq!(record1.fields.get("column_0").unwrap(), &FieldValue::String("John".to_string()));
        assert_eq!(record1.fields.get("column_1").unwrap(), &FieldValue::String("30".to_string()));
        assert_eq!(record1.fields.get("column_2").unwrap(), &FieldValue::String("New York".to_string()));
        
        // Read second record
        let record2 = reader.read().await.unwrap().unwrap();
        assert_eq!(record2.fields.get("column_0").unwrap(), &FieldValue::String("Jane".to_string()));
        assert_eq!(record2.fields.get("column_1").unwrap(), &FieldValue::String("25".to_string()));
        assert_eq!(record2.fields.get("column_2").unwrap(), &FieldValue::String("London".to_string()));
        
        // Read third record
        let record3 = reader.read().await.unwrap().unwrap();
        assert_eq!(record3.fields.get("column_0").unwrap(), &FieldValue::String("Bob".to_string()));
        assert_eq!(record3.fields.get("column_1").unwrap(), &FieldValue::String("35".to_string()));
        assert_eq!(record3.fields.get("column_2").unwrap(), &FieldValue::String("Paris".to_string()));
    ...
    async fn test_csv_custom_delimiter() {
        ...
        assert_eq!(record.fields.get("column_0").unwrap(), &FieldValue::String("John".to_string()));
        assert_eq!(record.fields.get("column_1").unwrap(), &FieldValue::String("30".to_string()));
        assert_eq!(record.fields.get("column_2").unwrap(), &FieldValue::String("New York".to_string()));
    }
...
{"name": "Bob", "age": 35, "city": "Paris"}"#;
        ...
        // Read first record
        let record1 = reader.read().await.unwrap().unwrap();
        assert_eq!(record1.fields.get("name").unwrap(), &FieldValue::String("John".to_string()));
        assert_eq!(record1.fields.get("age").unwrap(), &FieldValue::Integer(30));
        assert_eq!(record1.fields.get("active").unwrap(), &FieldValue::Boolean(true));
        
        // Read second record
        let record2 = reader.read().await.unwrap().unwrap();
        assert_eq!(record2.fields.get("name").unwrap(), &FieldValue::String("Jane".to_string()));
        assert_eq!(record2.fields.get("age").unwrap(), &FieldValue::Integer(25));
        assert_eq!(record2.fields.get("active").unwrap(), &FieldValue::Boolean(false));
        
        // Read third record (different schema)
        let record3 = reader.read().await.unwrap().unwrap();
        assert_eq!(record3.fields.get("name").unwrap(), &FieldValue::String("Bob".to_string()));
        assert_eq!(record3.fields.get("age").unwrap(), &FieldValue::Integer(35));
        assert_eq!(record3.fields.get("city").unwrap(), &FieldValue::String("Paris".to_string()));
    }
    ...
    async fn test_batch_reading() {
        ...
        assert_eq!(batch[0].fields.get("column_0").unwrap(), &FieldValue::String("1".to_string()));
        assert_eq!(batch[0].fields.get("column_1").unwrap(), &FieldValue::String("Alice".to_string()));
        
        assert_eq!(batch[1].fields.get("column_0").unwrap(), &FieldValue::String("2".to_string()));
        assert_eq!(batch[1].fields.get("column_1").unwrap(), &FieldValue::String("Bob".to_string()));
        
        assert_eq!(batch[2].fields.get("column_0").unwrap(), &FieldValue::String("3".to_string()));
        assert_eq!(batch[2].fields.get("column_1").unwrap(), &FieldValue::String("Charlie".to_string()));
    ...
    async fn test_file_watching_basic() {
        ...
        tokio::spawn(async move {
            ...
            fs::write(&file_path_clone, format!("{}{}", initial_content, additional_content)).unwrap();
        });
        ...
        for _attempt in 0..10 {
            ...
            if let Ok(Some(record)) = reader.read().await {
                if let Some(FieldValue::String(name)) = record.fields.get("column_0") {
                    if name == "Jane" {
                        assert_eq!(record.fields.get("column_1").unwrap(), &FieldValue::String("25".to_string()));
                        found_jane = true;
                        break;
                    }
                }
    ...
    async fn test_uri_parsing() {
        ...
        let config = FileSourceConfig::from_uri(
            "file:///data/test.csv?delimiter=;&header=false"
        ).unwrap();
