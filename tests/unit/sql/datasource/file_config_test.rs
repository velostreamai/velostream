...
mod file_config_tests {
    ...
    fn test_file_format_parsing() {
        ...
        assert_eq!("jsonlines".parse::<FileFormat>().unwrap(), FileFormat::JsonLines);
        assert_eq!("json_lines".parse::<FileFormat>().unwrap(), FileFormat::JsonLines);
        assert_eq!("json".parse::<FileFormat>().unwrap(), FileFormat::Json);
        assert_eq!("csvnoheader".parse::<FileFormat>().unwrap(), FileFormat::CsvNoHeader);
        assert_eq!("csv_no_header".parse::<FileFormat>().unwrap(), FileFormat::CsvNoHeader);
    ...
    fn test_config_validation_invalid() {
        ...
        assert!(config.validate().unwrap_err().contains("path cannot be empty"));
        
        // Zero buffer size
        let mut config = FileSourceConfig::new("/data/test.csv".to_string(), FileFormat::Csv);
        config.buffer_size = 0;
        assert!(config.validate().is_err());
        assert!(config.validate().unwrap_err().contains("Buffer size must be greater than 0"));
        
        // Zero polling interval
        let mut config = FileSourceConfig::new("/data/test.csv".to_string(), FileFormat::Csv);
        config.polling_interval_ms = Some(0);
        assert!(config.validate().is_err());
        assert!(config.validate().unwrap_err().contains("Polling interval must be greater than 0"));
        ...
        config.csv_quote = '"';
        assert!(config.validate().is_err());
        assert!(config.validate().unwrap_err().contains("delimiter and quote character cannot be the same"));
        
        // Escape same as delimiter
        let mut config = FileSourceConfig::new("/data/test.csv".to_string(), FileFormat::Csv);
        config.csv_escape = Some(',');
        assert!(config.validate().is_err());
        assert!(config.validate().unwrap_err().contains("escape character cannot be the same"));
    ...
    fn test_uri_parsing_invalid() {
        ...
        assert!(result.unwrap_err().contains("File URI must start with 'file://'"));
    ...
    fn test_uri_parsing_boolean_parameters() {
        ...
        for (uri, expected_watch) in configs {
            ...
            assert_eq!(config.watch_for_changes, expected_watch, "Failed for URI: {}", uri);
        }
    }
    ...
    fn test_uri_parsing_single_character_params() {
        ...
        let config = FileSourceConfig::from_uri(
            "file:///data/test.csv?delimiter=|&quote='"
        ).unwrap();
        ...
        let config = FileSourceConfig::from_uri(
            "file:///data/test.csv?delimiter=multi&quote=chars"
        ).unwrap();
