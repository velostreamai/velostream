...
use ferrisstreams::ferris::sql::execution::config::StreamingConfig;
...
async fn test_multi_source_watermark_coordination() {
    ...
    let fast_record = create_test_record_with_event_time(1, base_time + ChronoDuration::seconds(10), None);
    manager.update_watermark("fast_source", &fast_record);
    
    // Source 2: Slow source (older events) 
    let slow_record = create_test_record_with_event_time(2, base_time + ChronoDuration::seconds(5), None);
...
async fn test_window_processor_watermark_aware_processing() {
    ...
    let record1 = create_test_record_with_event_time(1, base_time + ChronoDuration::seconds(5), None);
    ...
    let record2 = create_test_record_with_event_time(2, base_time + ChronoDuration::seconds(8), None);
    ...
    let late_record = create_test_record_with_event_time(3, base_time - ChronoDuration::seconds(10), None);
...
async fn test_streaming_config_integration() {
    ...
    let custom_config = StreamingConfig::default()
        .with_watermarks()
        .with_late_data_strategy(LateDataStrategy::DeadLetterQueue {
...
async fn test_window_emission_timing_with_watermarks() {
    ...
    let record1 = create_test_record_with_event_time(1, base_time + ChronoDuration::seconds(6), None);
