...
fn test_create_table_with_emit_changes_parsing() {
    ...
    assert!(result.is_ok(), "Failed to parse CREATE TABLE with EMIT CHANGES: {:?}", result);
    ...
    match result.unwrap() {
        StreamingQuery::CreateTable {
            ...
            match *as_select {
                StreamingQuery::Select { emit_mode: nested_emit, .. } => {
                    assert_eq!(nested_emit, Some(EmitMode::Changes),
...
async fn test_ctas_executor_with_emit_changes() {
    ...
    // Test that CTAS executor preserves EMIT CHANGES
    let sql = r#"
    ...
    assert!(parsed.is_ok(), "Failed to parse CTAS with EMIT CHANGES");
    ...
    // This test verifies the SQL is valid and EMIT CHANGES is preserved
}
...
fn test_create_table_emit_final_with_window() {
    ...
    let result = parser.parse(sql);
    assert!(result.is_ok(), "Failed to parse CREATE TABLE with EMIT FINAL");
    ...
    match result.unwrap() {
        StreamingQuery::CreateTable {
            ...
            match *as_select {
                StreamingQuery::Select { window, emit_mode: nested_emit, .. } => {
                    assert!(window.is_some(), "EMIT FINAL requires WINDOW clause");
                    assert_eq!(nested_emit, Some(EmitMode::Final),
...
fn test_create_table_default_emit_mode() {
    ...
    match result.unwrap() {
        StreamingQuery::CreateTable {
            ...
            match *as_select {
                StreamingQuery::Select { group_by, emit_mode: nested_emit, .. } => {
...
fn test_ctas_emit_changes_cdc_semantics() {
    ...
    let result = parser.parse(sql);
    assert!(result.is_ok(), "Failed to parse complex CTAS with EMIT CHANGES");
