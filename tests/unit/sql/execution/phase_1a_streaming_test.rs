...
async fn test_correlation_id_support() {
    ...
    for message in messages {
        match &message {
            ExecutionMessage::StartJob { correlation_id: id, .. } => {
                assert_eq!(id, &correlation_id);
            }
            ExecutionMessage::AdvanceWatermark { correlation_id: id, .. } => {
                assert_eq!(id, &correlation_id);
            }
            ExecutionMessage::ProcessRecord { correlation_id: id, .. } => {
                assert_eq!(id, &correlation_id);
            }
            ExecutionMessage::FlushWindows { correlation_id: id, .. } => {
                assert_eq!(id, &correlation_id);
            }
...
async fn test_event_time_support_in_stream_record() {
    ...
    let record_with_event_time = StreamRecord::with_event_time(
        HashMap::new(),
        12345,
        0,
        0,
    ...
    let record = StreamRecord::default()
        .with_event_time_fluent(event_time);
...
mod performance_tests {
    ...
    async fn test_message_injection_performance() {
        ...
        assert!(messages_per_sec > 10_000.0, "Performance regression detected");
    }
    ...
    fn test_streamrecord_event_time_overhead() {
        ...
        assert!(records_per_sec > 100_000.0, "StreamRecord performance regression detected");
    }
}