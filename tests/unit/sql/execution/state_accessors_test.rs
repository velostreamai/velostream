...
fn test_get_group_states_empty() {
    ...
    assert!(
        states.is_empty(),
        "New engine should have no GROUP BY states"
    );
}
...
fn test_set_and_get_group_states() {
    ...
    assert!(
        retrieved_states.contains_key("test_query"),
        "Should contain test_query state"
    );
...
fn test_group_states_clone_pattern() {
    ...
    assert_eq!(
        final_states.len(),
        2,
        "Should have 2 GROUP BY states after sync"
    );
    assert!(
        final_states.contains_key("query1"),
        "Should still have query1"
    );
    assert!(
        final_states.contains_key("query2"),
        "Should have new query2"
    );
}
...
fn test_set_window_states_no_active_queries() {
    ...
    let window_state = WindowState {
        window_spec: WindowSpec::Tumbling {
            size: Duration::from_millis(5000),
            time_column: None,
        },
        buffer: vec![],
        last_emit: 0,
    ...
    let test_states = vec![("nonexistent_query".to_string(), window_state)];
    ...
    assert!(
        retrieved_states.is_empty(),
        "Should have no window states for non-existent queries"
    );
}
...
fn test_window_states_lifecycle() {
    ...
    assert!(
        ...
        "Initial window states should be empty"
    );
    ...
    assert!(
        after_empty_set.is_empty(),
        "After setting empty vec should still be empty"
    );
}
...
fn test_state_accessors_thread_safety_simulation() {
    ...
    assert_eq!(
        query1_state.groups.len(),
        1,
        "Should have 1 group after processing"
    );
}
...
fn test_multiple_state_updates() {
    ...
    for i in 0..5 {
        ...
        assert_eq!(
            current_states.len(),
            1,
            "Should have exactly 1 state after each update"
        );
        assert!(
            current_states.contains_key(&format!("query_{}", i)),
...
fn test_state_independence() {
    ...
    assert_eq!(
        final_group_states.len(),
        1,
        "GROUP BY state should remain after window state update"
    );

    // And: Window states should not be affected by GROUP BY
    let final_window_states = engine.get_window_states();
    assert_eq!(
        final_window_states.len(),
        0,
        "Window state should remain independent"
    );
}
