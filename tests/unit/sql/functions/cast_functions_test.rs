...
async fn test_cast_to_date_from_string() {
    ...
    for (date_str, should_succeed) in test_cases {
        let result = FieldValue::cast_to(FieldValue::String(date_str.to_string()), "DATE");
...
async fn test_cast_to_date_from_timestamp() {
    ...
    let result = FieldValue::cast_to(FieldValue::Timestamp(timestamp), "DATE");
...
async fn test_cast_to_timestamp_from_string() {
    ...
    for (timestamp_str, should_succeed) in test_cases {
        let result =
            FieldValue::cast_to(FieldValue::String(timestamp_str.to_string()), "TIMESTAMP");
...
async fn test_cast_to_timestamp_from_date() {
    ...
    let result = FieldValue::cast_to(FieldValue::Date(date), "TIMESTAMP");
...
async fn test_cast_to_timestamp_from_integer() {
    ...
    let result = FieldValue::cast_to(FieldValue::Integer(unix_timestamp), "TIMESTAMP");
...
async fn test_cast_to_decimal_from_various_types() {
    // Test from Integer
    let result = FieldValue::cast_to(FieldValue::Integer(42), "DECIMAL");
    assert!(result.is_ok(), "Should cast INTEGER to DECIMAL");
    assert_eq!(result.unwrap(), FieldValue::Decimal(Decimal::from(42)));

    // Test from Float
    let result = FieldValue::cast_to(FieldValue::Float(PI), "DECIMAL");
    ...
    // Test from String
    let result = FieldValue::cast_to(FieldValue::String("123.456".to_string()), "DECIMAL");
    assert!(
        result.is_ok(),
    ...
    // Test from Boolean
    let result = FieldValue::cast_to(FieldValue::Boolean(true), "DECIMAL");
    assert!(result.is_ok(), "Should cast BOOLEAN to DECIMAL");
    assert_eq!(result.unwrap(), FieldValue::Decimal(Decimal::ONE));

    let result = FieldValue::cast_to(FieldValue::Boolean(false), "DECIMAL");
    assert!(result.is_ok(), "Should cast BOOLEAN to DECIMAL");
    assert_eq!(result.unwrap(), FieldValue::Decimal(Decimal::ZERO));

    // Test invalid string
    let result = FieldValue::cast_to(FieldValue::String("not-a-number".to_string()), "DECIMAL");
    assert!(
        result.is_err(),
...
async fn test_cast_decimal_to_other_types() {
    ...
    let result = FieldValue::cast_to(FieldValue::Decimal(decimal_value), "INTEGER");
    assert!(result.is_ok(), "Should cast DECIMAL to INTEGER");
    assert_eq!(result.unwrap(), FieldValue::Integer(123));

    // Test DECIMAL to FLOAT
    let result = FieldValue::cast_to(FieldValue::Decimal(decimal_value), "FLOAT");
    ...
    let result = FieldValue::cast_to(FieldValue::Decimal(decimal_value), "STRING");
...
async fn test_cast_null_values() {
    ...
    for target_type in null_target_types {
        let result = FieldValue::cast_to(FieldValue::Null, target_type);
        assert!(
            result.is_ok(),
    ...
    let result = FieldValue::cast_to(FieldValue::Null, "STRING");
...
async fn test_cast_error_cases() {
    ...
    for (value, target_type) in invalid_casts {
        let result = FieldValue::cast_to(value.clone(), target_type);
        assert!(
            result.is_err(),
...
async fn test_cast_datetime_alias() {
    ...
    let result = FieldValue::cast_to(FieldValue::String(date_str.to_string()), "DATETIME");
...
async fn test_cast_numeric_alias() {
    ...
    let result = FieldValue::cast_to(FieldValue::String("123.456".to_string()), "NUMERIC");
...
async fn test_large_decimal_values() {
    ...
    let result = FieldValue::cast_to(FieldValue::String(large_decimal_str.to_string()), "DECIMAL");
...
async fn test_cast_display_string_format() {
    ...
    let result = FieldValue::cast_to(FieldValue::Date(date), "STRING");
    assert!(result.is_ok());
    if let Ok(FieldValue::String(s)) = result {
        assert_eq!(s, "2023-12-25");
    }

    let result = FieldValue::cast_to(FieldValue::Timestamp(timestamp), "STRING");
    assert!(result.is_ok());
    if let Ok(FieldValue::String(s)) = result {
        assert!(s.starts_with("2023-12-25 14:30:45"));
    }

    let result = FieldValue::cast_to(FieldValue::Decimal(decimal), "STRING");
    assert!(result.is_ok());
    if let Ok(FieldValue::String(s)) = result {
        assert_eq!(s, "123.456");
    }
