/*!
# FR-089: Inline PRIMARY KEY Annotation Tests

Tests for the SQL-standard PRIMARY KEY annotation syntax that allows marking
fields as Kafka message keys directly in SQL. This follows the syntax used
by Flink SQL, RisingWave, and Materialize.
...
/// Test single field PRIMARY KEY annotation
#[test]
fn test_single_key_annotation() {
    let parser = StreamingSqlParser::new();
    let result = parser.parse("SELECT symbol PRIMARY KEY, price FROM trades");
...
/// Test compound PRIMARY KEY annotation (multiple fields)
#[test]
fn test_compound_key_annotation() {
    let parser = StreamingSqlParser::new();
    let result = parser.parse(
        "SELECT region PRIMARY KEY, product PRIMARY KEY, SUM(qty) as total FROM orders GROUP BY region, product",
    );
...
/// Test PRIMARY KEY annotation with alias - alias name should be used
#[test]
fn test_key_annotation_with_alias() {
    let parser = StreamingSqlParser::new();
    let result = parser.parse("SELECT stock_symbol as symbol PRIMARY KEY, price FROM trades");
...
/// Test PRIMARY KEY annotation in CREATE STREAM AS SELECT
#[test]
fn test_key_annotation_in_create_stream() {
    let parser = StreamingSqlParser::new();
    let sql = r#"
        CREATE STREAM flagged_symbols AS
        SELECT symbol PRIMARY KEY, trade_count, total_volume
...
/// Test query without PRIMARY KEY annotation should have None key_fields
#[test]
fn test_no_key_annotation() {
    ...
    match query {
        StreamingQuery::Select { key_fields, .. } => {
            assert!(
                key_fields.is_none(),
                "key_fields should be None when no PRIMARY KEY annotation"
            );
        }
        _ => panic!("Expected Select query"),
    }
...
/// Test PRIMARY KEY annotation with expression (should use alias or generate name)
#[test]
...
/// Test PRIMARY KEY annotation case insensitivity
#[test]
...
/// Test PRIMARY KEY annotation with GROUP BY and WINDOW
#[test]
fn test_key_annotation_with_group_by_and_window() {
    ...
    match result.unwrap() {
        StreamingQuery::Select {
            ...
            // PRIMARY KEY annotation should be captured
            assert!(key_fields.is_some());
...
/// Test Display implementation outputs PRIMARY KEY annotation
#[test]
...
/// Test PRIMARY KEY annotation order is preserved
#[test]
fn test_key_annotation_order_preserved() {
    let parser = StreamingSqlParser::new();
    let result = parser.parse("SELECT region PRIMARY KEY, product PRIMARY KEY, qty FROM orders");
...
/// Test PRIMARY KEY annotation with qualified column names
#[test]
fn test_key_annotation_qualified_column() {
    let parser = StreamingSqlParser::new();
    let result = parser.parse("SELECT t.symbol PRIMARY KEY, t.price FROM trades t");
...
/// Test that PRIMARY KEY can appear after AS alias
#[test]
fn test_key_after_as_alias() {
    let parser = StreamingSqlParser::new();
    let result = parser.parse("SELECT stock_symbol AS sym PRIMARY KEY, price FROM trades");
