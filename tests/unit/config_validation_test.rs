...
fn test_timeout_boundary_values() {
    ...
    assert_eq!(zero_timeout_config.request_timeout_duration(), Duration::ZERO);
    ...
    assert_eq!(large_timeout_config.request_timeout_duration(), large_timeout);
    ...
    assert_eq!(reasonable_config.request_timeout_duration(), reasonable_timeout);
}
...
fn test_custom_property_validation() {
    ...
    let edge_case_config = config.clone()
...
fn test_consumer_config_boundary_values() {
    ...
    assert_eq!(boundary_config.request_timeout_duration(), Duration::from_secs(300));
}
...
fn test_performance_tuning_edge_cases() {
    ...
    let extreme_producer = producer_config
        .performance_tuning(1, 1);  // 1MB max message, 1KB socket buffer
        
    let extreme_consumer = consumer_config
        .performance_tuning(1, 1, 1);  // 1MB fetch, 1MB partition, 1KB socket
