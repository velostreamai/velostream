...
async fn benchmark_legacy_processing(record_count: usize) -> f64 {
    ...
    for i in 0..record_count {
        ...
        record.insert("amount".to_string(), FieldValue::Float(i as f64 * 10.5));
        record.insert("category".to_string(), FieldValue::String(format!("cat_{}", i % 10)));
        
        // Basic legacy processing
        let _id = record.get("id");
        let _amount = record.get("amount");
...
async fn measure_legacy_processing_latency() -> f64 {
    ...
    record.insert("data".to_string(), FieldValue::String("test_data".to_string()));
    
    // Simulate basic processing
    let _id = record.get("id");
    let _data = record.get("data");
...
async fn benchmark_stress_testing() {
    ...
    println!("🚀 Starting stress test: {} RPS for {:?}", TARGET_RPS, STRESS_DURATION);
    ...
    while start_time.elapsed() < STRESS_DURATION {
        ...
        for i in 0..100 { // Process in batches of 100
            let mut record = StreamRecord::new();
            record.insert("id".to_string(), FieldValue::Integer(records_processed as i64));
            record.insert("timestamp".to_string(), FieldValue::Integer(Utc::now().timestamp()));
            record.insert("value".to_string(), FieldValue::Float(i as f64));
            record.set_event_time(Some(Utc::now()));
            ...
            let result: Result<(), StreamingError> = circuit_breaker.execute(|| {
                ...
                let _processing = record.get("id").is_some() && record.get_event_time().is_some();
                Ok(())
            }).await;
...
async fn benchmark_financial_trading_scenario() -> f64 {
    ...
    for i in 0..TRADE_COUNT {
        let mut record = StreamRecord::new();
        record.insert("trade_id".to_string(), FieldValue::String(format!("T{}", i)));
        record.insert("symbol".to_string(), FieldValue::String("AAPL".to_string()));
        record.insert("price".to_string(), FieldValue::Float(150.0 + (i as f64 * 0.01)));
        record.insert("volume".to_string(), FieldValue::Integer(100 + (i as i64 * 10)));
        record.set_event_time(Some(Utc::now()));
        
        // High-frequency processing with circuit breaker protection
        let _: Result<(), StreamingError> = circuit_breaker.execute(|| {
            // Simulate trade validation and processing
            let _price = record.get("price");
            let _volume = record.get("volume");
...
async fn benchmark_iot_sensor_scenario() -> f64 {
    ...
    for i in 0..SENSOR_READINGS {
        let mut record = StreamRecord::new();
        record.insert("sensor_id".to_string(), FieldValue::String(format!("S{}", i % 1000)));
        record.insert("temperature".to_string(), FieldValue::Float(20.0 + (i as f64 % 10.0)));
        record.insert("humidity".to_string(), FieldValue::Float(50.0 + (i as f64 % 30.0)));
...
async fn benchmark_web_analytics_scenario() -> f64 {
    ...
    for i in 0..PAGE_VIEWS {
        let mut record = StreamRecord::new();
        record.insert("session_id".to_string(), FieldValue::String(format!("session_{}", i % 5000)));
        record.insert("page_url".to_string(), FieldValue::String(format!("/page/{}", i % 100)));
        record.insert("user_agent".to_string(), FieldValue::String("Mozilla/5.0".to_string()));
        record.insert("response_time".to_string(), FieldValue::Integer(100 + (i as i64 % 500)));
        ...
        let _: Result<(), StreamingError> = circuit_breaker.execute(|| {
            ...
            let _session = record.get("session_id");
            let _response_time = record.get("response_time");
