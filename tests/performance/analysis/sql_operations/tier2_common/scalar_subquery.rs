...
async fn test_scalar_subquery_performance() {
    ...
    let (adaptive_1c_throughput, _) =
        measure_adaptive_jp(records.clone(), SCALAR_SUBQUERY_SQL, 1).await;
    let (adaptive_4c_throughput, _) =
        measure_adaptive_jp(records.clone(), SCALAR_SUBQUERY_SQL, 4).await;

    println!(
        "🚀 BENCHMARK_RESULT | scalar_subquery | tier2 | SQL Sync: {:.0} | SQL Async: {:.0} | SimpleJp: {:.0} | TransactionalJp: {:.0} | AdaptiveJp (1c): {:.0} | AdaptiveJp (4c): {:.0}",
        sql_sync_throughput,
