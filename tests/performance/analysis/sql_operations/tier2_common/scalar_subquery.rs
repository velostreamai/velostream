...
/// Create the trades table for the scalar subquery test
fn create_trades_table() -> Arc<dyn UnifiedTable> {
    ...
    for i in 0..10 {
        ...
        fields.insert(
            "price".to_string(),
            FieldValue::Float(100.0 + (i % 10) as f64),
        );
        fields.insert(
            "timestamp".to_string(),
            FieldValue::Integer((i * 1000) as i64),
...
async fn test_scalar_subquery_performance() {
    ...
    let (sql_sync_throughput, _, _) = measure_sql_engine_sync(records.clone(), SCALAR_SUBQUERY_SQL).await;
    let (sql_async_throughput, _, _) = measure_sql_engine(records.clone(), SCALAR_SUBQUERY_SQL).await;
    let (simple_jp_throughput, _) = measure_v1(records.clone(), SCALAR_SUBQUERY_SQL).await;
    let (transactional_jp_throughput, _) = measure_transactional_jp(records.clone(), SCALAR_SUBQUERY_SQL).await;
    let (adaptive_1c_throughput, _) = measure_adaptive_jp(records.clone(), SCALAR_SUBQUERY_SQL, 1).await;
    let (adaptive_4c_throughput, _) = measure_adaptive_jp(records.clone(), SCALAR_SUBQUERY_SQL, 4).await;

    println!("🚀 BENCHMARK_RESULT | scalar_subquery | tier2 | SQL Sync: {:.0} | SQL Async: {:.0} | SimpleJp: {:.0} | TransactionalJp: {:.0} | AdaptiveJp (1c): {:.0} | AdaptiveJp (4c): {:.0}",
...
async fn measure_adaptive_jp(
    ...
    let _result = tokio::time::timeout(
        ...
        processor.process_job(
            ...
            (*query_arc).clone(),
            format!("scalar_subquery_adaptive_{}c_test", num_cores),
            shutdown_rx,
        ),
    )
    .await;
