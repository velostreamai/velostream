//! Correlated Subquery Performance Benchmark - Tier 3 Operation
//!
//! **Operation #13 in STREAMING_SQL_OPERATION_RANKING.md**
//! - **Tier**: Tier 3 (Advanced)
//! - **Probability**: 35% of production streaming SQL jobs
//! - **Use Cases**: Per-row evaluations, complex filtering with outer references
//!
//! Correlated subqueries evaluate the subquery for each outer row, making them
//! the slowest subquery type. They're used when the subquery needs to reference
...
fn generate_correlated_subquery_records(count: usize) -> Vec<StreamRecord> {
    (0..count)
        .map(|i| {
            let mut fields = HashMap::new();
            let employee_id = i as i64;
            let department_id = (i % 10) as i64; // 10 departments
            let salary = 50000.0 + (i % 100000) as f64;
            ...
            StreamRecord::new(fields).with_partition_from_key(&department_id.to_string(), 32)
        })
        .collect()
}
...
/// This requires evaluating the subquery for each outer row
const CORRELATED_SUBQUERY_SQL: &str = r#"
    SELECT
        employee_id,
        department_id,
        salary
...
async fn measure_sql_engine_sync(records: Vec<StreamRecord>, query: &str) -> (f64, usize, usize) {
    ...
    for record in records.iter() {
        ...
        match engine.execute_with_record_sync(&parsed_query, record) {
            ...
            Err(_e) => {
                // Expected: some employees earn less than avg
            }
        }
    }
