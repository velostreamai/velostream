...
use super::super::test_helpers::{get_perf_cardinality, get_perf_record_count, print_perf_config};
...
async fn test_group_by_continuous_performance() {
    ...
    println!("══════════════════════════════════════════════");
    println!("Operation #4: Tier 1 (93% probability)");
    println!("Use Case: Continuous aggregation, statistical summaries");
    println!();
    ...
    println!("   Query: GROUP BY with COUNT, AVG, MIN, MAX, SUM");
    println!("   Symbols: {} unique", cardinality);
    println!();
    ...
    let (simple_jp_throughput, simple_jp_produced) =
        measure_v1(records.clone(), GROUP_BY_CONTINUOUS_SQL).await;
    ...
    assert!(
        best.1 > 50_000.0,
        "GROUP BY (Continuous) performance below threshold: {:.0} rec/sec",
        best.1
    );
}
