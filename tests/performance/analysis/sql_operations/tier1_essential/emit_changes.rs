//! TUMBLING WINDOW with EMIT CHANGES Performance Benchmark - Tier 1 Operation
//!
//! **Operation #5 Alternative: EMIT CHANGES Variant**
//! - **Tier**: Tier 1 (Essential)
//! - **Probability**: 92% of production streaming SQL jobs
//! - **Use Cases**: Real-time dashboards, live metric updates, late-arriving data handling
//!
//! EMIT CHANGES provides streaming results (multiple emissions per window) vs EMIT FINAL
//! which provides a single finalized result. Lower throughput trade-off for real-time visibility.
...
/// Similar distribution to tumbling_window for comparison
...
/// Create the market_data table for the emit_changes test
...
/// SQL query with EMIT CHANGES semantics
/// Multiple emissions per window for real-time updates (vs EMIT FINAL for single final)
const EMIT_CHANGES_SQL: &str = r#"
...
#[serial_test::serial]
async fn test_emit_changes_performance() {
    // Validate SQL query
    validate_sql_query(EMIT_CHANGES_SQL);

    let record_count = get_perf_record_count();
    let records = generate_emit_changes_records(record_count);

    let (sql_sync_throughput, _, _) =
        measure_sql_engine_sync(records.clone(), EMIT_CHANGES_SQL).await;
    let (sql_async_throughput, _, _) = measure_sql_engine(records.clone(), EMIT_CHANGES_SQL).await;
    let (simple_jp_throughput, _) = measure_v1(records.clone(), EMIT_CHANGES_SQL).await;
    let (transactional_jp_throughput, _) =
        measure_transactional_jp(records.clone(), EMIT_CHANGES_SQL).await;
    let (adaptive_1c_throughput, _) =
        measure_adaptive_jp(records.clone(), EMIT_CHANGES_SQL, 1).await;
    let (adaptive_4c_throughput, _) =
        measure_adaptive_jp(records.clone(), EMIT_CHANGES_SQL, 4).await;

    println!(
        "🚀 BENCHMARK_RESULT | emit_changes | tier1 | SQL Sync: {:.0} | SQL Async: {:.0} | SimpleJp: {:.0} | TransactionalJp: {:.0} | AdaptiveJp (1c): {:.0} | AdaptiveJp (4c): {:.0}",
        sql_sync_throughput,
...
async fn measure_v1(records: Vec<StreamRecord>, query: &str) -> (f64, usize) {
    ...
    let _result = tokio::time::timeout(
        ...
        processor.process_job(
            ...
            (*query_arc).clone(),
            "emit_changes_v1_test".to_string(),
            shutdown_rx,
            None,
        ),
    )
...
async fn measure_transactional_jp(records: Vec<StreamRecord>, query: &str) -> (f64, usize) {
    ...
    let _result = tokio::time::timeout(
        ...
        processor.process_job(
            ...
            (*query_arc).clone(),
            "emit_changes_transactional_test".to_string(),
            shutdown_rx,
            None,
        ),
    )
...
async fn measure_adaptive_jp(
    ...
    let _result = tokio::time::timeout(
        ...
        processor.process_job(
            ...
            (*query_arc).clone(),
            format!("emit_changes_adaptive_{}c_test", num_cores),
            shutdown_rx,
            None,
        ),
    )
