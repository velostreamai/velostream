...
mod performance_benchmarks {
    ...
    fn benchmark_f64_aggregation() {
        ...
        println!("Throughput: {:.0} records/sec", BENCHMARK_SIZE as f64 / duration.as_secs_f64());
        println!("Sum: {:.4}", sum);
        println!("Avg: {:.4}", avg);
        println!("Min: {:.4}", min);
        println!("Max: {:.4}", max);
    ...
    fn benchmark_scaled_i64_aggregation() {
        ...
        for &value in &data {
            ...
            count += 1;
            if scaled_value.scaled_value < min.scaled_value { min = scaled_value; }
            if scaled_value.scaled_value > max.scaled_value { max = scaled_value; }
        }
    ...
    fn benchmark_rust_decimal_aggregation() {
        ...
        for &value in &data {
            ...
            sum += decimal_value;
            count += 1;
            if decimal_value < min { min = decimal_value; }
            if decimal_value > max { max = decimal_value; }
        }
    ...
    fn benchmark_precision_comparison() {
        ...
        let problematic_values = vec![
            0.1, 0.2, 0.3,           // Classic decimal representation issues
            1.0/3.0,                  // Repeating decimal  
            99.999,                   // Near boundary values
            0.0001, 0.0002, 0.0003,   // Small values
        ];
        ...
        println!("f64 == expected:     {}", (f64_sum - expected).abs() < 1e-15);
        println!("i64 ~= expected:     {}", (i64_sum.to_f64() - expected).abs() < 1e-4);
        println!("i128 ~= expected:    {}", (i128_sum.to_f64() - expected).abs() < 1e-8);
        println!("Decimal ~= expected: {}", (decimal_sum.to_f64().unwrap() - expected).abs() < 1e-15);
    }
    ...
    fn benchmark_financial_calculation_patterns() {
        ...
        println!("f64 total:     ${:.2} (time: {:?})", f64_total, f64_duration);
        println!("i64 total:     ${:.2} (time: {:?})", i64_total.to_f64(), i64_duration);
        println!("Decimal total: ${:.2} (time: {:?})", decimal_total.to_f64().unwrap(), decimal_duration);
        
        // Performance comparison
        println!("\nPerformance relative to f64:");
        println!("i64 overhead:     {:.2}x", i64_duration.as_nanos() as f64 / f64_duration.as_nanos() as f64);
        println!("Decimal overhead: {:.2}x", decimal_duration.as_nanos() as f64 / f64_duration.as_nanos() as f64);
    }
}
...
mod integration_tests {
    ...
    fn test_aggregation_accuracy_real_world() {
        ...
        println!("Processing {} small financial transactions", transactions.len());
        ...
        println!("\nResults:");
        println!("f64 - Sum: ${:.6}, Avg: ${:.6}, Time: {:?}", f64_sum, f64_avg, f64_time);
        println!("i64 - Sum: ${:.6}, Avg: ${:.6}, Time: {:?}", i64_sum.to_f64(), i64_avg.to_f64(), i64_time);
        println!("Dec - Sum: ${:.6}, Avg: ${:.6}, Time: {:?}", 
        ...
        println!("decimal sum diff: ${:.6}", (decimal_sum.to_f64().unwrap_or(0.0) - f64_sum).abs());
        
        println!("\nPerformance vs f64:");
        println!("i64 performance: {:.2}x", i64_time.as_nanos() as f64 / f64_time.as_nanos() as f64);
        println!("Decimal performance: {:.2}x", decimal_time.as_nanos() as f64 / f64_time.as_nanos() as f64);
        ...
        if sum_difference > 1e-6 {
            println!("\n⚠️  PRECISION ISSUE DETECTED: f64 has ${:.6} error in sum", sum_difference);
        }
    }
}