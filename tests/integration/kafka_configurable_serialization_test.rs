...
mod phase2_integration_tests {
    ...
    fn test_sql_with_clause_to_producer_integration() {
        ...
        let producer_builder =
            ConfigurableKafkaProducerBuilder::<String, CustomerSpendingEvent>::from_sql_config(
                "localhost:9092",
    ...
    fn test_avro_sql_with_clause_integration() {
        ...
        sql_with_params.insert(
            ...
            "customer-spending-value".to_string(),
        );
    ...
    fn test_different_serialization_combinations() {
        ...
        for (key_format, value_format) in test_cases {
            ...
            let config = SerializationConfig::from_sql_params(&sql_params).expect(&format!(
    ...
    fn test_financial_precision_configuration() {
        ...
        financial_params.insert(
            "bootstrap.servers".to_string(),
            "financial-kafka:9092".to_string(),
        );
        ...
        let financial_producer =
            ConfigurableKafkaProducerBuilder::<String, FinancialEvent>::from_sql_config(
                "financial-kafka:9092",
        ...
        assert_eq!(
            consumer_config.custom_properties.get("acks"),
            Some(&"all".to_string())
        );
        assert_eq!(
            consumer_config.custom_properties.get("enable.idempotence"),
            Some(&"true".to_string())
        );

        let producer_config = financial_producer.serialization_config.unwrap();
        assert_eq!(
            producer_config.custom_properties.get("compression.type"),
            Some(&"lz4".to_string())
        );
        assert_eq!(
            producer_config.custom_properties.get("retries"),
    ...
    fn test_high_throughput_configuration() {
        ...
        high_throughput_params.insert(
            "bootstrap.servers".to_string(),
            "iot-cluster:9092".to_string(),
        );
        ...
        let iot_producer =
            ConfigurableKafkaProducerBuilder::<Vec<u8>, IoTSensorData>::from_sql_config(
                "iot-cluster:9092",
                "sensor-data",
    ...
    fn test_error_propagation_in_pipeline() {
        ...
        let producer_result =
            ConfigurableKafkaProducerBuilder::<String, TransactionEvent>::from_sql_config(
                "localhost:9092",
                "test-topic",
                &invalid_params,
            );
        ...
        // Both should have similar error messages - check that they contain expected error information
        assert!(
            consumer_result.is_err(),
            "Consumer should fail with invalid serialization format"
        );
        assert!(
            producer_result.is_err(),
            "Producer should fail with invalid serialization format"
        );

        // Verify the error types are SerializationError (which indicates format validation failed)
        if let Err(consumer_err) = consumer_result {
            let error_msg = format!("{:?}", consumer_err);
            assert!(
                error_msg.contains("serialization") || error_msg.contains("format"),
                "Consumer error should relate to serialization: {}",
                error_msg
            );
        }

        if let Err(producer_err) = producer_result {
            let error_msg = format!("{:?}", producer_err);
            assert!(
                error_msg.contains("serialization") || error_msg.contains("format"),
                "Producer error should relate to serialization: {}",
                error_msg
            );
        }
    }
