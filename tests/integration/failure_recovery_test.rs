...
async fn test_network_partition_recovery_with_retry_logic() {
    ...
    producer.send(Some(&"key1".to_string()), &initial_message, Headers::new(), None).await
    ...
    for attempt in 1..=max_retries {
        ...
        match timeout(
            ...
            producer.send(Some(&format!("retry-key-{}", attempt)), &partition_message, Headers::new(), None)
        ).await {
            Ok(Ok(_)) => {
            ...
            Ok(Err(e)) => {
                retry_count += 1;
                _last_error = Some(e);
                println!("❌ Retry attempt {} failed, waiting {:?}", attempt, retry_delay);
                sleep(retry_delay).await;
            }
            Err(_) => {
                retry_count += 1;
                println!("⏰ Retry attempt {} timed out, waiting {:?}", attempt, retry_delay);
                sleep(retry_delay).await;
            }
        }
    }
    ...
    assert!(retry_count > 0 || producer.send(Some(&"test".to_string()), &partition_message, Headers::new(), None).await.is_ok(),
    ...
    let recovery_result = producer.send(
...
async fn test_consumer_graceful_degradation() {
    ...
    for i in 1..=3 {
        ...
        producer.send(Some(&format!("key{}", i)), &message, Headers::new(), None).await
            .expect("Failed to send pre-failure message");
    }
    
    producer.flush(5000).expect("Failed to flush pre-failure messages");
    sleep(Duration::from_secs(1)).await;
    ...
    for attempt in 1..=max_attempts {
        match timeout(Duration::from_millis(500), consumer.poll(Duration::from_millis(100))).await {
            Ok(Ok(message)) => {
                received_count += 1;
                println!("✅ Received message {}: {:?}", received_count, message.value().content);
            }
            Ok(Err(KafkaClientError::Timeout)) => {
                println!("⚠️  Consumer timeout on attempt {} (expected during degradation)", attempt);
            }
            Ok(Err(e)) => {
                println!("⚠️  Consumer error on attempt {}: {:?}", attempt, e);
            }
            Err(_) => {
                println!("⚠️  Poll timeout on attempt {} (graceful degradation)", attempt);
            }
        }
    }
...
async fn test_retry_with_exponential_backoff() {
    ...
    for attempt in 1..=max_retries {
        ...
        match producer.send(Some(&"retry-key".to_string()), &message, Headers::new(), None).await {
            Ok(_) => {
    ...
    assert!(retry_attempts.load(Ordering::Relaxed), "Should have attempted retries");
}
...
async fn test_partial_broker_failure_scenarios() {
    ...
    for i in 0..3 {
        match timeout(
            ...
            Ok(Ok(_)) => {
                ...
                println!("✅ Mixed producer message {} succeeded despite partial failure", i);
            }
            Ok(Err(e)) => {
    ...
    assert_eq!(working_success_count, 3, "Working producer should succeed all operations");
    ...
    println!("📊 Partial failure stats: Working={}/3, Mixed={}/3", 
...
async fn test_connection_pool_recovery() {
    ...
    let topic = format!("recovery-pool-{}", Uuid::new_v4());
    let mut producers = Vec::new();
    
    // Create multiple producers to potentially exhaust connection pool
    for i in 0..5 {
        let producer = KafkaProducer::<String, TestMessage, _, _>::new(
            "localhost:9092",
            &topic,
        ...
        match producer {
            Ok(p) => {
                producers.push(p);
                println!("✅ Created producer {}", i + 1);
            }
            Err(e) => {
                println!("❌ Failed to create producer {}: {:?}", i + 1, e);
                break;
            }
        }
    }

    // Test that we can send messages with multiple producers
    let message = TestMessage::new(1, "Pool test message");
    let mut success_count = 0;
    
    for (i, producer) in producers.iter().enumerate() {
        match producer.send(
            Some(&format!("pool-key-{}", i)),
            &message,
            Headers::new(),
            None
        ).await {
            Ok(_) => {
                success_count += 1;
                println!("✅ Producer {} sent message successfully", i + 1);
            }
            Err(e) => {
                println!("❌ Producer {} failed to send: {:?}", i + 1, e);
            }
        }
    }
    ...
    drop(producers);
    println!("🧹 Dropped all producers, testing recovery...");
    
    sleep(Duration::from_millis(100)).await;
    
    // Create new producer after cleanup
    let recovery_producer = KafkaProducer::<String, TestMessage, _, _>::new(
        "localhost:9092",
        &topic,
        JsonSerializer,
        JsonSerializer,
    ).expect("Failed to create recovery producer");

    let recovery_message = TestMessage::new(2, "Recovery test message");
    match recovery_producer.send(
        Some(&"recovery-key".to_string()),
        &recovery_message,
        Headers::new(),
        None
    ).await {
        Ok(_) => println!("✅ Recovery producer succeeded"),
        Err(e) => println!("❌ Recovery producer failed: {:?}", e),
    }

    assert!(success_count > 0, "At least some producers should succeed");
}
...
async fn test_graceful_shutdown_recovery() {
    ...
    match producer.flush(5000) {
        Ok(_) => {
            ...
            assert!(flush_duration < Duration::from_secs(6), "Flush should complete within timeout");
        }
        Err(e) => {
    ...
    match consumer.poll(Duration::from_secs(2)).await {
        Ok(message) => {
            println!("✅ Received message before shutdown: {:?}", message.value().content);
