...
async fn test_ktable_basic_creation() {
    ...
    match result {
        ...
        Err(e) => {
            println!("⚠️  Kafka not available for KTable basic creation test: {:?}", e);
            println!("   This test requires a running Kafka instance at {}", KAFKA_BROKERS);
        }
    }
}
...
async fn test_ktable_from_consumer() {
    ...
    match consumer_result {
        ...
        Err(e) => {
            println!("⚠️  Kafka not available for KTable from consumer test: {:?}", e);
            println!("   This test requires a running Kafka instance at {}", KAFKA_BROKERS);
        }
    }
}
...
async fn test_ktable_transformations() {
    ...
    match ktable_result {
        Ok(ktable) => {
            ...
            let name_map: HashMap<String, String> = ktable.map_values(|profile| profile.name.clone());
            assert!(name_map.is_empty());
            
            // Test filter transformation
            let adults: HashMap<String, UserProfile> = ktable.filter(|_key, profile| profile.age >= 18);
        ...
        Err(e) => {
            println!("⚠️  Kafka not available for KTable transformations test: {:?}", e);
            println!("   This test requires a running Kafka instance at {}", KAFKA_BROKERS);
        }
    }
}
...
async fn test_ktable_with_producer_simulation() {
    ...
    match ktable_result {
        ...
        Err(e) => {
            println!("⚠️  Kafka not available for KTable producer simulation test: {:?}", e);
            println!("   This test requires a running Kafka instance at {}", KAFKA_BROKERS);
        }
    }
}
...
async fn test_ktable_multiple_types() {
    ...
    match (user_table_result, order_table_result) {
        (Ok(user_table), Ok(order_table)) => {
            ...
            let user_emails: HashMap<String, String> = user_table.map_values(|user| user.email.clone());
        ...
        (Err(e), _) | (_, Err(e)) => {
            println!("⚠️  Kafka not available for KTable multiple types test: {:?}", e);
            println!("   This test requires a running Kafka instance at {}", KAFKA_BROKERS);
        }
    }
}
...
async fn test_ktable_configuration_options() {
    ...
    match ktable_result {
        ...
        Err(e) => {
            println!("⚠️  Kafka not available for KTable configuration test: {:?}", e);
            println!("   This test requires a running Kafka instance at {}", KAFKA_BROKERS);
        }
    }
}