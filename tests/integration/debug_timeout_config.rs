...
async fn test_timeout_configuration_debug() {
    ...
    println!("  transaction_timeout: {:?}", default_config.transaction_timeout);
    
    // Test 2: Transactional with default timeout (should not adjust)
    let transactional_config = ProducerConfig::new("localhost:9092", "test-topic")
        .transactional("test-tx-id");
    println!("\nTransactional config (default timeout):");
    println!("  message_timeout: {:?}", transactional_config.message_timeout);
    println!("  transaction_timeout: {:?}", transactional_config.transaction_timeout);
    ...
    println!("  transaction_timeout: {:?}", short_tx_config.transaction_timeout);
    ...
    println!("  transaction_timeout: {:?}", tx_timeout_only.transaction_timeout);
    
    // Verify the constraint is satisfied
    assert!(short_tx_config.message_timeout <= short_tx_config.transaction_timeout,
        "message_timeout ({:?}) should be <= transaction_timeout ({:?})",
        short_tx_config.message_timeout, short_tx_config.transaction_timeout);
    
    assert!(tx_timeout_only.message_timeout <= tx_timeout_only.transaction_timeout,
        "message_timeout ({:?}) should be <= transaction_timeout ({:?})",
