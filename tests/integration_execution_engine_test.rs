...
use ferrisstreams::ferris::{
    serialization::{JsonFormat, InternalValue},
    sql::{
...
fn create_market_data_record(
    ...
    record.insert("symbol".to_string(), InternalValue::String(symbol.to_string()));
    ...
    record.insert("timestamp".to_string(), InternalValue::Integer(chrono::Utc::now().timestamp_millis()));
    record
}
...
async fn test_string_functions() {
    ...
    let sql = "SELECT UPPER(symbol) as upper_symbol, LENGTH(symbol) as symbol_length FROM market_data";
    let records = vec![
        create_market_data_record("aapl", 150.0, 151.0, 100, 200),
    ];
...
async fn test_null_handling() {
    ...
    let mut record = HashMap::new();
    record.insert("symbol".to_string(), serde_json::Value::String("TEST".to_string()));
...
async fn test_complex_expression() {
    ...
    let records = vec![
        create_market_data_record("AAPL", 150.5, 151.7, 100, 200),
    ];
...
async fn test_end_to_end_integration() {
    ...
    println!("   GOOGL: ${:.2} potential profit", googl_result["potential_profit"]);
    println!("   TSLA: ${:.2} potential profit", tsla_result["potential_profit"]);
}