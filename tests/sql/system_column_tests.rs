...
mod tests {
    ...
    fn test_system_column_aliasing() {
        let parser = StreamingSqlParser::new();
        let result = parser.parse("SELECT _timestamp AS ts, _offset AS msg_offset, _partition AS part FROM orders");
    ...
    async fn test_system_column_execution() {
        ...
        let parser = StreamingSqlParser::new();
        let query = parser.parse("SELECT customer_id, _timestamp, _offset, _partition FROM orders").unwrap();
    ...
    async fn test_system_column_with_aliases() {
        ...
        let parser = StreamingSqlParser::new();
        let query = parser.parse("SELECT _timestamp AS event_time, _partition AS kafka_partition FROM orders").unwrap();
    ...
    async fn test_system_column_in_where_clause() {
        ...
        let parser = StreamingSqlParser::new();
        let query = parser.parse("SELECT customer_id FROM orders WHERE _partition = 0").unwrap();
    ...
    async fn test_mixed_regular_and_system_columns() {
        ...
        let parser = StreamingSqlParser::new();
        let query = parser.parse("SELECT customer_id, amount, _timestamp, _offset FROM orders").unwrap();
        
        // Create test record
        let mut record = HashMap::new();
        record.insert("customer_id".to_string(), Value::Number(serde_json::Number::from(101)));
        record.insert("amount".to_string(), Value::Number(serde_json::Number::from_f64(59.99).unwrap()));
    ...
    async fn test_wildcard_does_not_include_system_columns() {
        ...
        // Create test record
        let mut record = HashMap::new();
        record.insert("customer_id".to_string(), Value::Number(serde_json::Number::from(202)));
        record.insert("amount".to_string(), Value::Number(serde_json::Number::from_f64(149.99).unwrap()));
    ...
    async fn test_system_columns_case_insensitive() {
        ...
        for (i, query_str) in queries.iter().enumerate() {
            ...
            // Create test record
            let mut record = HashMap::new();
            record.insert("test_id".to_string(), Value::Number(serde_json::Number::from(i as i64)));
    ...
    async fn test_csas_with_system_columns() {
        ...
        // Create test record
        let mut record = HashMap::new();
        record.insert("customer_id".to_string(), Value::Number(serde_json::Number::from(303)));
        record.insert("amount".to_string(), Value::Number(serde_json::Number::from_f64(75.50).unwrap()));
    ...
    fn test_system_column_in_expression() {
        ...
        for query in queries {
            let result = parser.parse(query);
            assert!(result.is_ok(), "Failed to parse query with system column in expression: {}", query);
        }
    }
    ...
    fn test_system_column_reserved_names() {
        ...
        let system_columns = vec!["_timestamp", "_offset", "_partition", "_TIMESTAMP", "_OFFSET", "_PARTITION"];
        
        for col in system_columns {
            let query = format!("SELECT {} FROM orders", col);
            let result = parser.parse(&query);
            assert!(result.is_ok(), "System column {} should parse correctly", col);
