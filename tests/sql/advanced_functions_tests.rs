...
mod tests {
    ...
    fn test_describe_command() {
        ...
        assert!(result.is_ok(), "Failed to parse DESCRIBE: {:?}", result.err());
        
        match result.unwrap() {
            StreamingQuery::Show { resource_type, pattern } => {
                assert_eq!(resource_type, ShowResourceType::Describe { name: "orders".to_string() });
    ...
    fn test_advanced_aggregation_functions() {
        ...
        for func in functions {
            ...
            let result = parser.parse(&query);
            assert!(result.is_ok(), "Failed to parse query with {}: {:?}", func, result.err());
    ...
    fn test_timestamp_function() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse TIMESTAMP() function: {:?}", result.err());
    ...
    fn test_cast_function() {
        ...
        for (cast_expr, expected_type) in test_cases {
            ...
            let result = parser.parse(&query);
            assert!(result.is_ok(), "Failed to parse CAST function {}: {:?}", cast_expr, result.err());
    ...
    fn test_split_function() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse SPLIT function: {:?}", result.err());
    ...
    fn test_join_function() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse JOIN function: {:?}", result.err());
    ...
    fn test_complex_query_with_multiple_functions() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse complex query with multiple functions: {:?}", result.err());
        
        match result.unwrap() {
            StreamingQuery::Select { fields, where_clause, group_by, order_by, limit, .. } => {
                ...
                let function_names: Vec<String> = fields.iter()
                    .filter_map(|field| {
                        match field {
                            SelectField::Expression { expr, .. } => {
                                match expr {
    ...
    fn test_show_enhanced_commands() {
        ...
        let test_cases = vec![
            ...
            ("SHOW STATUS analytics", ShowResourceType::JobStatus { name: Some("analytics".to_string()) }),
            ("SHOW VERSIONS order_monitor", ShowResourceType::JobVersions { name: "order_monitor".to_string() }),
            ("SHOW METRICS", ShowResourceType::JobMetrics { name: None }),
            ("SHOW METRICS analytics", ShowResourceType::JobMetrics { name: Some("analytics".to_string()) }),
        ];
        
        for (query_str, expected_resource_type) in test_cases {
            let result = parser.parse(query_str);
            assert!(result.is_ok(), "Failed to parse {}: {:?}", query_str, result.err());
                    ...
                    SelectField::Expression { expr, alias } => {
                        ...
                        match expr {
                            Expr::Function { name, args } => {
                                ...
                                match &args[0] {
                                    Expr::Function { name: inner_name, args: inner_args } => {
                                        assert_eq!(inner_name.to_uppercase(), "SPLIT");
                                        assert_eq!(inner_args.len(), 2);
                                    }
                                    _ => panic!("Expected SPLIT function as first argument to CAST"),
                                }
                            }
                            _ => panic!("Expected CAST function"),
                        }
    ...
    fn test_case_insensitive_function_names() {
        ...
        for query_str in test_cases {
            let result = parser.parse(query_str);
            assert!(result.is_ok(), "Failed to parse case-insensitive function: {}", query_str);
    ...
    fn test_function_error_cases() {
        ...
        for query_str in valid_cases {
            ...
            assert!(result.is_ok(), "Parser should accept function call: {}", query_str);
        }
    }
    ...
    fn test_show_jobs_compatibility() {
        ...
        let query = "SHOW JOBS";
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse SHOW JOBS: {:?}", result.err());
    ...
    fn test_deployment_with_advanced_functions() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse deployment with advanced functions: {:?}", result.err());
        
        match result.unwrap() {
            StreamingQuery::DeployJob { name, version, query, strategy, .. } => {
                ...
                match *query {
                    StreamingQuery::Select { fields, .. } => {
                        ...
                        let has_functions = fields.iter().any(|field| {
                            match field {
                                SelectField::Expression { expr, .. } => {
                        ...
                        assert!(has_functions, "Expected to find function calls in deployed query");
                    }
