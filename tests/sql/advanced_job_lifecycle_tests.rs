...
mod tests {
    ...
    fn test_pause_job_basic() {
        ...
        assert!(result.is_ok(), "Failed to parse PAUSE JOB: {:?}", result.err());
    ...
    fn test_resume_job_basic() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse RESUME JOB: {:?}", result.err());
    ...
    fn test_deploy_job_basic() {
        ...
        let query = "DEPLOY JOB order_monitor VERSION '1.2.0' AS SELECT * FROM orders WHERE amount > 100";
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse DEPLOY JOB: {:?}", result.err());
    ...
    fn test_deploy_job_with_strategy_blue_green() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse DEPLOY JOB with BLUE_GREEN: {:?}", result.err());
        
        match result.unwrap() {
            StreamingQuery::DeployJob { name, version, strategy, .. } => {
    ...
    fn test_deploy_job_with_strategy_canary() {
        ...
        let query = "DEPLOY JOB analytics VERSION '2.1.0' AS SELECT * FROM orders STRATEGY CANARY(25)";
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse DEPLOY JOB with CANARY: {:?}", result.err());
    ...
    fn test_deploy_job_with_strategy_rolling() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse DEPLOY JOB with ROLLING: {:?}", result.err());
    ...
    fn test_deploy_job_with_strategy_replace() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse DEPLOY JOB with REPLACE: {:?}", result.err());
    ...
    fn test_deploy_job_with_properties() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse DEPLOY JOB with properties: {:?}", result.err());
        
        match result.unwrap() {
            StreamingQuery::DeployJob { name, version, properties, .. } => {
    ...
    fn test_rollback_job_basic() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse ROLLBACK JOB: {:?}", result.err());
    ...
    fn test_rollback_job_with_version() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse ROLLBACK JOB with version: {:?}", result.err());
    ...
    fn test_case_insensitive_lifecycle_commands() {
        ...
        for (query_str, expected_type) in test_cases {
            ...
            assert!(result.is_ok(), "Failed to parse case-insensitive {}: {}", expected_type, query_str);
    ...
    fn test_invalid_canary_percentage() {
        ...
        assert!(result.is_ok(), "Parser should accept any number, validation handles range");
    }
    ...
    fn test_deploy_job_complex_underlying_query() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse DEPLOY JOB with complex SELECT: {:?}", result.err());
        
        match result.unwrap() {
            StreamingQuery::DeployJob { name, version, query, .. } => {
                assert_eq!(name, "complex_analytics");
                assert_eq!(version, "2.5.0");
                
                // Verify the underlying query is properly parsed
                match *query {
                    StreamingQuery::Select { fields, where_clause, group_by, having, order_by, limit, .. } => {
    ...
    fn test_get_columns_for_lifecycle_commands() {
        ...
        for query_str in lifecycle_queries {
            ...
            assert!(columns.is_empty(), "Lifecycle command should not reference columns: {}", query_str);
        }
    ...
    fn test_has_window_for_lifecycle_commands() {
        ...
        for query_str in no_window_queries {
            let result = parser.parse(query_str);
            assert!(result.is_ok());
            assert!(!result.unwrap().has_window(), "Command should not have window: {}", query_str);
        }
        ...
        assert!(result.is_ok());
        assert!(result.unwrap().has_window(), "DEPLOY JOB should inherit window from underlying query");
    }
}