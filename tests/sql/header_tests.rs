...
mod tests {
    ...
    async fn test_header_function_execution() {
        ...
        assert_eq!(
            output.get("content_type").unwrap().as_str().unwrap(),
            "application/json"
        );
    ...
    async fn test_has_header_function() {
        ...
        assert_eq!(
            output.get("has_content_type").unwrap().as_bool().unwrap(),
            true
        );
    ...
    async fn test_header_with_csas() {
        ...
        record.insert(
            "amount".to_string(),
            Value::Number(serde_json::Number::from_f64(199.99).unwrap()),
        );
        ...
        assert_eq!(
            output.get("source").unwrap().as_str().unwrap(),
            "mobile-app"
        );
        assert_eq!(
            output.get("app_version").unwrap().as_str().unwrap(),
            "2.1.0"
        );
    }
    ...
    async fn test_mixed_headers_and_system_columns() {
        ...
        assert_eq!(
            output.get("request_id").unwrap().as_str().unwrap(),
            "req-789"
        );
    ...
    async fn test_header_case_sensitivity() {
        ...
        let output = rx.try_recv().unwrap();
        assert_eq!(
            output.get("ct").unwrap().as_str().unwrap(),
            "application/json"
        );
