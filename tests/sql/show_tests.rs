...
mod tests {
    ...
    fn test_show_streams() {
        ...
        assert!(result.is_ok(), "Failed to parse SHOW STREAMS: {:?}", result.err());
    ...
    fn test_list_streams() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse LIST STREAMS: {:?}", result.err());
    ...
    fn test_show_schema() {
        ...
        match result.unwrap() {
            StreamingQuery::Show { resource_type, pattern } => {
                assert_eq!(resource_type, ShowResourceType::Schema { name: "orders".to_string() });
                assert!(pattern.is_none());
            }
            _ => panic!("Expected Show query"),
        }
    ...
    fn test_show_properties() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse SHOW PROPERTIES: {:?}", result.err());
        
        match result.unwrap() {
            StreamingQuery::Show { resource_type, pattern } => {
                assert_eq!(resource_type, ShowResourceType::Properties { 
    ...
    fn test_show_partitions() {
        ...
        match result.unwrap() {
            StreamingQuery::Show { resource_type, pattern } => {
                assert_eq!(resource_type, ShowResourceType::Partitions { name: "orders".to_string() });
                assert!(pattern.is_none());
            }
            _ => panic!("Expected Show query"),
        }
    ...
    fn test_show_streams_with_like_pattern() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse SHOW STREAMS with LIKE pattern: {:?}", result.err());
    ...
    fn test_show_tables_with_like_pattern() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse SHOW TABLES with LIKE pattern");
    ...
    fn test_case_insensitive_show_commands() {
        ...
        for query in queries {
            let result = parser.parse(query);
            assert!(result.is_ok(), "Failed to parse case-insensitive query: {}", query);
    ...
    fn test_show_schema_missing_name() {
        ...
        let result = parser.parse(query);
        assert!(result.is_err(), "Should fail when SHOW SCHEMA is missing resource name");
    }
    ...
    fn test_show_properties_missing_args() {
        ...
        let result = parser.parse(query);
        assert!(result.is_err(), "Should fail when SHOW PROPERTIES is missing arguments");
    }
    ...
    fn test_show_partitions_missing_name() {
        ...
        let result = parser.parse(query);
        assert!(result.is_err(), "Should fail when SHOW PARTITIONS is missing resource name");
    }
}