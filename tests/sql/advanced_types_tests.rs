...
fn create_test_record_with_advanced_types() -> StreamRecord {
    ...
    struct_data.insert("first_name".to_string(), FieldValue::String("John".to_string()));
    struct_data.insert("last_name".to_string(), FieldValue::String("Doe".to_string()));
...
async fn test_array_functions() {
    // Test ARRAY creation
    let results = execute_query("SELECT ARRAY(1, 2, 3, 4) as new_array FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
    // Note: The result would be serialized to JSON, so we'd need to check the structure
    
    // Test ARRAY_LENGTH
    let results = execute_query("SELECT ARRAY_LENGTH(numbers) as array_len FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
    assert_eq!(results[0]["array_len"], 3);
    
    // Test ARRAY_CONTAINS
    let results = execute_query("SELECT ARRAY_CONTAINS(numbers, 2) as contains_two FROM test_stream").await.unwrap();
    assert_eq!(results[0]["contains_two"], true);
    
    let results = execute_query("SELECT ARRAY_CONTAINS(numbers, 5) as contains_five FROM test_stream").await.unwrap();
    assert_eq!(results[0]["contains_five"], false);
    
    // Test mixed type array
    let results = execute_query("SELECT ARRAY('hello', 42, true) as mixed_array FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
}
...
async fn test_map_functions() {
    // Test MAP creation
    let results = execute_query("SELECT MAP('name', 'Alice', 'age', 25) as person_map FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
    
    // Test MAP_KEYS
    let results = execute_query("SELECT MAP_KEYS(metadata) as meta_keys FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
    // Keys should be returned as an array
    
    // Test MAP_VALUES  
    let results = execute_query("SELECT MAP_VALUES(metadata) as meta_values FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
...
async fn test_struct_functions() {
    ...
    let results = execute_query("SELECT STRUCT('Alice', 30, true) as person_struct FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
...
async fn test_advanced_type_operations() {
    ...
    let results = execute_query("SELECT CONCAT('Numbers: ', numbers) as array_concat FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
    // Should get something like "Numbers: [1, 2, 3]"
    
    // Test CONCAT with maps
    let results = execute_query("SELECT CONCAT('Metadata: ', metadata) as map_concat FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
    
    // Test CONCAT with structs
    let results = execute_query("SELECT CONCAT('Person: ', person) as struct_concat FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
}
...
async fn test_null_handling_advanced_types() {
    ...
    let results = execute_query("SELECT ARRAY_LENGTH(NULL) as null_array_len FROM test_stream").await.unwrap();
    assert!(results[0]["null_array_len"].is_null());
    
    // Test ARRAY_CONTAINS with null array
    let results = execute_query("SELECT ARRAY_CONTAINS(NULL, 1) as null_contains FROM test_stream").await.unwrap();
    assert!(results[0]["null_contains"].is_null());
    
    // Test MAP_KEYS with null
    let results = execute_query("SELECT MAP_KEYS(NULL) as null_map_keys FROM test_stream").await.unwrap();
...
async fn test_type_errors() {
    ...
    let result = execute_query("SELECT ARRAY_LENGTH(name) as invalid_array_len FROM test_stream").await;
    assert!(result.is_err());
    
    // Test ARRAY_CONTAINS with non-array
    let result = execute_query("SELECT ARRAY_CONTAINS(name, 'test') as invalid_contains FROM test_stream").await;
    assert!(result.is_err());
    ...
    let result = execute_query("SELECT MAP('key1', 'value1', 'key2') as invalid_map FROM test_stream").await;
    assert!(result.is_err());
}
...
async fn test_complex_expressions() {
    ...
    let results = execute_query("SELECT ARRAY_LENGTH(ARRAY(1, 2, 3, 4, 5)) as expr_array_len FROM test_stream").await.unwrap();
    assert_eq!(results[0]["expr_array_len"], 5);
    
    // Test array contains with expression
    let results = execute_query("SELECT ARRAY_CONTAINS(ARRAY('a', 'b', 'c'), UPPER('a')) as expr_contains FROM test_stream").await.unwrap();
...
async fn test_comparison_operations() {
    ...
    let results = execute_query("SELECT ARRAY(1, 2, 3) = ARRAY(1, 2, 3) as arrays_equal FROM test_stream").await.unwrap();
    assert_eq!(results[0]["arrays_equal"], true);
    
    let results = execute_query("SELECT ARRAY(1, 2, 3) = ARRAY(3, 2, 1) as arrays_not_equal FROM test_stream").await.unwrap();
    assert_eq!(results[0]["arrays_not_equal"], false);
    
    // Maps should be comparable  
    let results = execute_query("SELECT MAP('a', 1, 'b', 2) = MAP('b', 2, 'a', 1) as maps_equal FROM test_stream").await.unwrap();
...
async fn test_coalesce_with_advanced_types() {
    ...
    let results = execute_query("SELECT COALESCE(NULL, ARRAY(1, 2, 3)) as coalesce_array FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
    
    // Test COALESCE with maps
    let results = execute_query("SELECT COALESCE(NULL, MAP('key', 'value')) as coalesce_map FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
    
    // Test NULLIF with arrays
    let results = execute_query("SELECT NULLIF(ARRAY(1, 2, 3), ARRAY(1, 2, 3)) as nullif_array FROM test_stream").await.unwrap();
...
async fn test_array_edge_cases() {
    // Empty array
    let results = execute_query("SELECT ARRAY() as empty_array FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
    
    let results = execute_query("SELECT ARRAY_LENGTH(ARRAY()) as empty_array_len FROM test_stream").await.unwrap();
    assert_eq!(results[0]["empty_array_len"], 0);
    
    // Array with nulls
    let results = execute_query("SELECT ARRAY(1, NULL, 3) as array_with_nulls FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
    
    let results = execute_query("SELECT ARRAY_LENGTH(ARRAY(1, NULL, 3)) as array_with_nulls_len FROM test_stream").await.unwrap();
...
async fn test_performance_with_large_structures() {
    ...
    let results = execute_query("SELECT ARRAY(ARRAY(1, 2), ARRAY(3, 4), ARRAY(5, 6)) as nested_arrays FROM test_stream").await.unwrap();
    assert_eq!(results.len(), 1);
}