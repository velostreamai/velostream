...
mod tests {
    ...
    fn test_substring_function_basic() {
        ...
        assert!(result.is_ok(), "Failed to parse SUBSTRING function: {:?}", result.err());
    ...
    fn test_substring_function_without_length() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse SUBSTRING without length: {:?}", result.err());
    ...
    fn test_json_extract_function() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse JSON_EXTRACT function: {:?}", result.err());
    ...
    fn test_json_value_function() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse JSON_VALUE function: {:?}", result.err());
    ...
    fn test_json_extract_with_array_access() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse JSON_EXTRACT with array access: {:?}", result.err());
    ...
    fn test_complex_query_with_string_and_json_functions() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse complex query with string/JSON functions: {:?}", result.err());
        
        match result.unwrap() {
            StreamingQuery::Select { fields, where_clause, order_by, limit, .. } => {
    ...
    fn test_nested_string_json_functions() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse nested string/JSON functions: {:?}", result.err());
                        ...
                        match expr {
                            Expr::Function { name, args } => {
                                ...
                                match &args[0] {
                                    ...
                                    _ => panic!("Expected JSON_VALUE function as first argument to SUBSTRING"),
                                }
                            }
                            _ => panic!("Expected SUBSTRING function"),
                        }
    ...
    fn test_job_deployment_with_json_processing() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse job deployment with JSON processing: {:?}", result.err());
            ...
            StreamingQuery::DeployJob { name, version, query, strategy, .. } => {
                ...
                match *query {
                    StreamingQuery::Select { fields, .. } => {
                        ...
                        let has_json_functions = fields.iter().any(|field| {
                            match field {
                                SelectField::Expression { expr, .. } => {
                        ...
                        assert!(has_json_functions, "Expected to find JSON function calls in deployed query");
                    }
                        ...
                        SelectField::Expression { expr, .. } => {
                            match expr {
                                Expr::Function { name, .. } => {
                                    ...
                                    assert!(
                                        upper_name == "SUBSTRING" || 
                                        upper_name == "JSON_EXTRACT",
                                        "Expected SUBSTRING or JSON_EXTRACT, got: {}", upper_name
                                    );
                                }
    ...
    fn test_kafka_payload_processing_example() {
        ...
        let result = parser.parse(query);
        assert!(result.is_ok(), "Failed to parse Kafka payload processing query: {:?}", result.err());
        ...
        match result.unwrap() {
            StreamingQuery::Select { fields, where_clause, .. } => {
                ...
                let has_system_columns = fields.iter().any(|field| {
                    match field {
                ...
                assert!(has_system_columns, "Expected system columns for Kafka metadata");
