...
fn main() {
    ...
    println!(
        "OLD (Double Conversion): {:8.1}ns per record",
        avg_double_conversion
    );
    println!(
        "NEW (Direct Usage):      {:8.1}ns per record",
        avg_direct_usage
    );
    ...
    for (type_name, field_value) in test_values {
        ...
        println!(
            "{:13}: {:6.1}ns per double conversion",
    ...
    println!(
        "✓ Achieved {:.1}x performance improvement",
        improvement_factor
    );
